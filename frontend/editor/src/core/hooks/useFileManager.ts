...
import { PDFEliteFileStub, PDFEliteFile } from "@app/types/fileContext";
...
export const useFileManager = () => {
  ...
  const convertToFile = useCallback(
    async (fileStub: PDFEliteFileStub): Promise<File> => {
      if (!indexedDB) {
  ...
  const loadRecentFiles = useCallback(async (): Promise<PDFEliteFileStub[]> => {
    setLoading(true);
    try {
      if (!indexedDB) {
        return [];
      ...
      const PDFEliteFileStubs = await fileStorage.getLeafPDFEliteFileStubs();
      const remoteIdSet = new Set(
        PDFEliteFileStubs.map((stub) => stub.remoteStorageId).filter(
          (id): id is number => typeof id === "number",
        ),
      );
      let combinedStubs = PDFEliteFileStubs;
      ...
      if (shouldFetchServerFiles) {
        try {
          ...
          const serverStubs: PDFEliteFileStub[] = [];
          ...
          const updatedLocalStubs = PDFEliteFileStubs.map((stub) => {
        ...
        if (config?.storageShareLinksEnabled === true) {
          try {
            ...
            const sharedStubs: PDFEliteFileStub[] = [];
  ...
  const handleRemoveFile = useCallback(
    async (
      index: number,
      files: PDFEliteFileStub[],
      setFiles: (files: PDFEliteFileStub[]) => void,
  ...
  const createFileSelectionHandlers = useCallback(
    (selectedFiles: FileId[], setSelectedFiles: (files: FileId[]) => void) => {
      ...
      const selectMultipleFiles = async (
        files: PDFEliteFileStub[],
        onPDFEliteFilesSelect: (PDFEliteFiles: PDFEliteFile[]) => void,
      ) => {
        if (selectedFiles.length === 0) return;

        try {
          // Filter by UUID and load full PDFEliteFile objects directly
          const selectedFileObjects = files.filter((f) =>
            selectedFiles.includes(f.id),
          );

          const PDFEliteFiles = await Promise.all(
          ...
          onPDFEliteFilesSelect(PDFEliteFiles);
          clearSelection();
        } catch (error) {
