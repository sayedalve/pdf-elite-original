...
export function useAnnotationMenuHandlers({
  ...
  const annotationType = useMemo((): AnnotationType => {
    ...
    if (
      ...
      if (type === PdfAnnotationSubtype.SQUARE) {
        const shapeObj = annotation?.object as any;
        if (shapeObj.opacity !== undefined && shapeObj.opacity < 1) {
    ...
    (
      ...
      if (target === "stroke") {
        ...
        if (
          ...
          // Dispatch color change event so the top toolbar and defaults stay in sync
          const mappedType =
