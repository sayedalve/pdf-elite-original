 ...
 * Contains all data needed for both PDFEliteFile and PDFEliteFileStub
 */
const THUMBNAIL_TTL_MS = 30 * 24 * 60 * 60 * 1000; // 30 days

export interface StoredPDFEliteFileRecord extends BaseFileMetadata {
  data: ArrayBuffer;
  fileId: FileId; // Matches runtime PDFEliteFile.fileId exactly
  quickKey: string; // Matches runtime PDFEliteFile.quickKey exactly
  thumbnail?: string;
  ...
  // See PDFEliteFileStub.classificationLabels.
...
export function legacyDerivedFromTool(
  record: StoredPDFEliteFileRecord,
): boolean | undefined {
...
class FileStorageService {
  ...
  private isThumbnailFresh(record: StoredPDFEliteFileRecord): boolean {
  ...
  private async bumpThumbnailTTL(ids: FileId[], clear = false): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      ids.forEach((id) => {
        ...
        req.onsuccess = () => {
          const record = req.result as StoredPDFEliteFileRecord | undefined;
          if (!record) return;
          if (clear) {
  ...
  /**
   * Store a PDFEliteFile with its metadata from PDFEliteFileStub
   */
  async storePDFEliteFile(
    PDFEliteFile: PDFEliteFile,
    stub: PDFEliteFileStub,
  ): Promise<void> {
    const db = await this.getDatabase();
    const arrayBuffer = await PDFEliteFile.arrayBuffer();

    const record: StoredPDFEliteFileRecord = {
      id: PDFEliteFile.fileId,
      fileId: PDFEliteFile.fileId, // Explicit field for clarity
      quickKey: PDFEliteFile.quickKey,
      name: PDFEliteFile.name,
      type: PDFEliteFile.type,
      size: PDFEliteFile.size,
      lastModified: PDFEliteFile.lastModified,
      ...
      originalFileId: stub.originalFileId ?? PDFEliteFile.fileId,
  ...
  /**
   * Get PDFEliteFile with full data - for loading into workbench
   */
  async getPDFEliteFile(id: FileId): Promise<PDFEliteFile | null> {
    const db = await this.getDatabase();
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const record = request.result as StoredPDFEliteFileRecord | undefined;
        if (!record) {
          resolve(null);
          return;
        }
        ...
        // Convert to PDFEliteFile with preserved IDs
        const PDFEliteFile = createPDFEliteFile(file, record.fileId);
        resolve(PDFEliteFile);
      };
    });
  }

  /**
   * Get multiple PDFEliteFiles - for batch loading
   */
  async getPDFEliteFiles(ids: FileId[]): Promise<PDFEliteFile[]> {
    const results = await Promise.all(
      ids.map((id) => this.getPDFEliteFile(id)),
    );
    return results.filter((file): file is PDFEliteFile => file !== null);
  }

  /**
   * Get PDFEliteFileStub (metadata only) - for UI browsing
   */
  async getPDFEliteFileStub(id: FileId): Promise<PDFEliteFileStub | null> {
    const db = await this.getDatabase();
  ...
  /**
   * Get all PDFEliteFileStubs (metadata only) - for FileManager browsing
   */
  async getAllPDFEliteFileStubs(): Promise<PDFEliteFileStub[]> {
    const db = await this.getDatabase();
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        if (cursor) {
          const record = cursor.value as StoredPDFEliteFileRecord;
  ...
  ): Promise<PDFEliteFileStub[]> {
    const stubs = await this.getAllPDFEliteFileStubs();
    return stubs
  ...
  /**
   * Get leaf PDFEliteFileStubs only - for unprocessed files
   */
  async getLeafPDFEliteFileStubs(): Promise<PDFEliteFileStub[]> {
    const db = await this.getDatabase();
    ...
    return new Promise((resolve, reject) => {
      ...
      const store = transaction.objectStore(this.storeName);
      const request = store.openCursor();
      const leafStubs: PDFEliteFileStub[] = [];
      const tobump: FileId[] = [];
      const toexpire: FileId[] = [];
  ...
  /**
   * Delete PDFEliteFile - single operation, no sync issues
   */
  async deletePDFEliteFile(id: FileId): Promise<void> {
    const db = await this.getDatabase();
  ...
  /**
   * Delete multiple PDFEliteFiles in a single transaction
   */
  async deleteMultiplePDFEliteFiles(ids: FileId[]): Promise<void> {
    if (ids.length === 0) return;
    const db = await this.getDatabase();
    ...
    return new Promise((resolve, _reject) => {
      try {
        ...
        getRequest.onsuccess = () => {
          const record = getRequest.result as StoredPDFEliteFileRecord;
          if (record) {
  ...
  async markFileAsProcessed(fileId: FileId): Promise<boolean> {
    try {
      ...
      const record = await new Promise<StoredPDFEliteFileRecord | undefined>(
        (resolve, reject) => {
  ...
  async persistVersionedOutputs(
    ...
    const allV1 = outputPDFEliteFileStubs.every(
    ...
    await Promise.all(
      outputPDFEliteFiles.map((file, i) =>
        this.storePDFEliteFile(file, outputPDFEliteFileStubs[i]).catch(
          (error) =>
            console.error(
  ...
  async updateFileMetadata(
    fileId: FileId,
    updates: Partial<StoredPDFEliteFileRecord>,
  ): Promise<boolean> {
    try {
      const db = await this.getDatabase();
