...
export const getFileFromSelection = (
  explicit: PDFEliteFile | null | undefined,
  fileId: FileId | null,
  selectors: { getFile: (id: FileId) => PDFEliteFile | undefined | null },
): PDFEliteFile | null => {
  if (explicit) return explicit;
  if (!fileId) return null;
  return (selectors.getFile(fileId) as PDFEliteFile | undefined | null) ?? null;
};
