...
export function FilesPageProvider({ children }: { children: React.ReactNode }) {
  ...
  const [allFiles, setAllFiles] = useState<PDFEliteFileStub[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const localStubs = await fileStorage.getAllPDFEliteFileStubs();
  ...
  const fileMap = useMemo(() => {
    const map = new Map<FileId, PDFEliteFileStub>();
  ...
  const moveFilesTo = useCallback(
    async (fileIds: FileId[], folderId: FolderId | null) => {
      ...
      const stubs = fileIds
        ...
        .filter((s): s is PDFEliteFileStub => Boolean(s));
        ...
        try {
          for (const stub of localOnly) {
            ...
            for (const chainStub of chain) {
              fileActions.updatePDFEliteFileStub(chainStub.id, {
