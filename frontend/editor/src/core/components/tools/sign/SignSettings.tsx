...
const SignSettings = ({
  ...
  const signatureKeysByType = useMemo(() => {
    ...
    return {
      ...
      image: imageKey,
      text: textKey,
    } as Record<SavedSignatureType, string | null>;
  }, [
    canvasSignatureData,
    imageSignatureData,
  ...
  const saveSignatureToLibrary = useCallback(
    async (
      ...
      if (isSavedSignatureLimitReached) {
        return { success: false, reason: "limit", isAtCapacity: true };
      }
  ...
  useEffect(() => {
    ...
    if (parameters.signatureDataUrl !== newSignatureData) {
      onParameterChange?.("signatureDataUrl", newSignatureData);
    }
  }, [
    parameters.signatureType,
    parameters.signatureDataUrl,
    canvasSignatureData,
    imageSignatureData,
    onParameterChange,
  ]);
