  ...
  await page.route(
    ...
    (route: Route) =>
      route.fulfill({
        json: [
          {
            alias: "Anthony PDFElite",
            source: "WINDOWS_STORE",
            subject: "CN=Anthony PDFElite",
            issuer: "CN=Anthony PDFElite",
            subjectCommonName: "Anthony PDFElite",
            issuerCommonName: "Anthony PDFElite",
            serialNumber: "abc123",
            keyAlgorithm: "RSA",
...
test.describe("CertSign tool - hardware signing (desktop)", () => {
  ...
  test("offers 'This device' and lists Windows store certificates", async ({
    ...
    ).toHaveValue(/Anthony PDFElite/, { timeout: 10_000 });
  });
});
