...
describe("injectOg (build-time prerender)", () => {
  ...
  it("replaces title + description and injects exactly one of each", () => {
    ...
    expect(out).toContain("<title>Compress - PDF Elite</title>");
  ...
  it("uses absolute URLs + og:url + secure_url when a canonical base is given", () => {
    const out = injectOg(TEMPLATE, entry, {
      ogBase: "https://PDFElitepdf.com",
      pageUrlPath: "/compress",
    });
    expect(out).toContain(
      '<meta property="og:image" content="https://PDFElitepdf.com/og_images/compress.png" />',
    );
    expect(out).toContain(
      '<meta property="og:url" content="https://PDFElitepdf.com/compress" />',
    );
  ...
  it("uses ogTitle for the social card but title for the <title> tag", () => {
    const out = injectOg(
      TEMPLATE,
      {
        ...
        title: "PDFElite - Edit any PDF. Govern every PDF.",
    ...
    expect(out).toContain(
      "<title>PDFElite - Edit any PDF. Govern every PDF.</title>",
    );
    expect(out).toContain(
  ...
  it("writes a flat file per single-segment route and a nested file (absolute base) per sub-route", async () => {
    ...
    const manifest = {
      ...
      byTool: {
        ...
        "/settings/people": {
          image: "/og_images/home.png",
          title: "People Settings - PDF Elite",
          description: "p",
        },
      },
      byPath: {
    ...
    expect(nested).toContain("<title>People Settings - PDF Elite</title>");
