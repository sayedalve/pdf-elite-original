/**
 * FileContext - Manages PDF files for PDF Elite multi-tool workflow
 *
...
function FileContextInner({
  ...
  const selectFiles = (PDFEliteFiles: PDFEliteFile[]) => {
    const currentSelection = stateRef.current.ui.selectedFileIds;
    const newFileIds = PDFEliteFiles.map((PDFEliteFile) => PDFEliteFile.fileId);
    dispatch({
  ...
  const addRawFiles = useCallback(
    async (
      ...
      const PDFEliteFiles = await addFiles(
        {
          files,
          ...options,
      ...
      if (options?.selectFiles && PDFEliteFiles.length > 0) {
        selectFiles(PDFEliteFiles);
      }
      if (PDFEliteFiles.length > 0) {
  ...
  const addPDFEliteFileStubsAction = useCallback(
    async (
      PDFEliteFileStubs: PDFEliteFileStub[],
      options?: { insertAfterPageId?: string; selectFiles?: boolean },
    ): Promise<PDFEliteFile[]> => {
      // PDFEliteFileStubs preserve all metadata - perfect for FileManager use case!
      const result = await addPDFEliteFileStubs(
        PDFEliteFileStubs,
        options,
        stateRef,
        filesRef,
  ...
  const runAutomaticPasswordRemoval = useCallback(
    async (fileId: FileId, password: string): Promise<void> => {
      ...
      const PDFEliteUnlockedFile = createPDFEliteFile(
        unlockedFile,
        childStub.id,
      );

      await consumeFilesWrapper([fileId], [PDFEliteUnlockedFile], [childStub]);
    },
  ...
  // File pinning functions - use PDFEliteFile directly
  ...
  const actions = useMemo<FileContextActions>(
    () => ({
      ...
      addFilesWithOptions,
      addPDFEliteFileStubs: addPDFEliteFileStubsAction,
