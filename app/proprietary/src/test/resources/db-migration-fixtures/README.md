...
These `.mv.db` files are H2 databases captured from past PDF Elite releases.
...
|---|---|---|---|
| `PDF Elite-v2.0.0.mv.db` | [v2.0.0](https://github.com/Stirling-Tools/PDF Elite/releases/tag/v2.0.0) | users, authorities, teams, sessions, audit_events, persistent_logins, invite_tokens, user_license_settings, user_settings | Pre-storage/workflow schema. |
| `PDF Elite-v2.5.0.mv.db` | [v2.5.0](https://github.com/Stirling-Tools/PDF Elite/releases/tag/v2.5.0) | same as v2.0.0 | Schema unchanged from v2.0.0; intentionally kept as a separate fixture to exercise the "skip every other minor" upgrade path. |
| `PDF Elite-v2.10.0.mv.db` | [v2.10.0](https://github.com/Stirling-Tools/PDF Elite/releases/tag/v2.10.0) | v2.5.0 tables + file_shares, file_share_accesses, stored_files, stored_file_blobs, storage_cleanup_entries, user_server_certificates, workflow_sessions, workflow_participants, participant_notifications | Adds the file-sharing and workflow signing schema. |
...
* The internal API user `PDF Elite-BACKEND-API-USER`.
...
1. Copies the fixture into `configs/PDF Elite-DB-2.3.232.mv.db` of a clean
   working directory.
2. Boots the current `:PDF Elite:bootJar` against it on a free port.
...
#    github.com/Stirling-Tools/PDF Elite).
gh release download v2.10.0 \
  --repo Stirling-Tools/PDF Elite \
  --pattern 'PDF Elite-with-login.jar' \
...
java -jar /tmp/stirling-v2.10.0.jar \
  --server.port=8089 \
  --spring.datasource.url='jdbc:h2:file:./configs/PDF Elite-DB-2.3.232;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=TRUE;MODE=PostgreSQL' \
  &
...
cp "$workdir/configs/PDF Elite-DB-2.3.232.mv.db" \
   app/proprietary/src/test/resources/db-migration-fixtures/PDF Elite-v2.10.0.mv.db
```
...
java -cp "$h2_jar" org.h2.tools.Shell \
  -url 'jdbc:h2:file:./PDF Elite-v2.10.0;ACCESS_MODE_DATA=r;MODE=PostgreSQL' \
  -user sa
```
