...
interface OverlayPdfsSettingsProps extends ToolAutomationSettingsProps<OverlayPdfsParameters> {}

const OverlayPdfsSettings = ({
  parameters,
  onChange,
  disabled = false,
}: OverlayPdfsSettingsProps) => {
  const onParameterChange = <K extends keyof OverlayPdfsParameters>(
    key: K,
  ...
  return (
    <Stack gap="md">
      <Stack gap="xs">
        <Text size="sm" fw={500}>
          {t("overlay-pdfs.mode.label", "Overlay Mode")}
        </Text>
        <Select
          data={[
            {
      ...
      <Stack gap="xs">
        <Text size="sm" fw={500}>
          {t("overlay-pdfs.position.label", "Overlay Position")}
        </Text>
        <SegmentedControl
      ...
      {parameters.overlayMode === "FixedRepeatOverlay" && (
        <>
          ...
          <Stack gap="xs">
            <Text size="sm" fw={500}>
              {t("overlay-pdfs.counts.label", "Overlay Counts")}
            </Text>
