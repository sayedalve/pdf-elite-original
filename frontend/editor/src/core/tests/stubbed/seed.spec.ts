...
/**
 * Seed test for pdf-elite E2E tests.
...
test.describe("pdf-elite seed", () => {
  test("seed - app loads", async ({ page }) => {
    // Navigate to the pdf-elite frontend
    await page.goto("/");
    ...
    await expect(
      page
        .locator(
          '.h-screen, .mobile-layout, [data-testid="dashboard"], img[alt*="PDFElite"]',
        )
        .first(),
    ).toBeVisible({ timeout: 15000 });

    // Verify the title contains PDF Elite
    await expect(page).toHaveTitle(/PDFElite/i);
  });
});
