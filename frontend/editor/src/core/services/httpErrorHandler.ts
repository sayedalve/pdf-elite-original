...
const POST_LOGIN_REDIRECT_STORAGE_KEY = "PDFElite_post_login_path";
...
const LOGIN_REDIRECT_THROTTLE_KEY = "PDFElite_last_401_redirect";
...
export async function handleHttpError(error: any): Promise<boolean> {
  ...
  if (status === 401) {
    ...
    if (!isAuthPage && !skipAuthRedirect) {
      ...
      try {
        hadStoredJwt = Boolean(localStorage.getItem("PDFElite_jwt"));
      } catch {
