...
import { PDFEliteFileStub, createPDFEliteFile } from "@app/types/fileContext";
...
export class ZipFileService {
  ...
  /**
   * Check if a PDFEliteFileStub represents a ZIP file (for UI checks without loading full file)
   */
  public isZipFileStub(stub: PDFEliteFileStub): boolean {
    const hasValidType =
   ...
   * @param zipStub - The PDFEliteFileStub for the ZIP (contains metadata to preserve)
   * @returns Object with success status, extracted stubs, and any errors
   */
  async extractAndStoreFilesWithHistory(
    zipFile: File,
    zipStub: PDFEliteFileStub,
  ): Promise<{
    success: boolean;
    extractedStubs: PDFEliteFileStub[];
    errors: string[];
  }> {
    const result = {
      success: false,
      extractedStubs: [] as PDFEliteFileStub[],
      errors: [] as string[],
    };
    ...
    try {
      ...
      for (const extractedFile of extractionResult.extractedFiles) {
        try {
          ...
          // Create PDFEliteFile
          const newPDFEliteFile = createPDFEliteFile(extractedFile);

          // Create PDFEliteFileStub with ZIP's history metadata
          const stub: PDFEliteFileStub = {
            id: newPDFEliteFile.fileId,
            name: extractedFile.name,
            size: extractedFile.size,
            type: extractedFile.type,
            lastModified: extractedFile.lastModified,
            quickKey: newPDFEliteFile.quickKey,
            createdAt: Date.now(),
            isLeaf: true,
          ...
          // Store in IndexedDB
          await fileStorage.storePDFEliteFile(newPDFEliteFile, stub);
