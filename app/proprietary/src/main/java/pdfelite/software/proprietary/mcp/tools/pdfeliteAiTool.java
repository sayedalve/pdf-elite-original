...
import pdfelite.software.proprietary.mcp.McpTool;
...
import pdfelite.software.proprietary.mcp.catalog.OperationMeta;
...
public class pdfeliteAiTool implements McpTool {
    ...
    public String description() {
        return "Invoke a pdfelite AI agent capability (Q&A about a PDF, edit-plan generation,"
                + " inline comments, math audit, draft-spec helper). Call"
                + " pdfelite_describe_operation with the chosen capability id to get its"
    ...
    public ObjectNode inputSchema() {
        ...
        fileId.put(
                "description",
                "Reference to a previously-uploaded PDF in pdfelite's job store. Required for"
    ...
    public ObjectNode call(JsonNode arguments, McpCallContext context) {
        ...
        try {
            String response = client.post(meta.endpointPath(), body, context.pdfeliteUserId());
