...
export default function WorkbenchBar({
  ...
  const exportTargetIds: string[] =
    currentView === "viewer"
      ...
      : selectedFileIds.length > 0
        ? selectedFileIds
        : activeFiles.filter(isPDFEliteFile).map((f) => f.fileId);
  ...
  const sectionsWithButtons = useMemo(() => {
    return SECTION_ORDER.map((section) => {
      // Only show context sections if they match the activeMode from the left rail
      if (
        section !== "navigation" &&
        section !== "other" &&
        section !== "search" &&
  ...
  const handleExportAll = useCallback(
    async (forceNewFile = false) => {
      if (currentView === "viewer") {
        ...
        const stub = isPDFEliteFile(fileToExport)
          ? selectors.getPDFEliteFileStub(fileToExport.fileId)
          : undefined;
        try {
          ...
          if (!forceNewFile && !result.cancelled && stub && result.savedPath) {
            fileActions.updatePDFEliteFileStub(stub.id, {
      ...
      const stubs = filesToExport.map((file) =>
        isPDFEliteFile(file)
          ? selectors.getPDFEliteFileStub(file.fileId)
          : undefined,
      );
  ...
  const handleClose = useCallback(async () => {
    if (currentView === "fileEditor") {
      ...
      if (file && isPDFEliteFile(file)) {
        // Pick the next file to show before removing, so the sidebar stays in sync.
        const remaining = activeFiles.filter(
          (f) => isPDFEliteFile(f) && f.fileId !== file.fileId,
        );
        const nextFile = remaining.find(isPDFEliteFile) ?? null;
  ...
  // View options — "Active Files" is hidden when in viewer mode because
  // the DocumentTabBar already provides file switching there.
  const viewOptions: ViewOption[] = [
    ...
    // Only show "Active Files" when NOT in the viewer (i.e., in file editor or page editor)
    ...(currentView !== "viewer"
      ? [
          {
  ...
  return (
    ...
    >
      ...
      <div className="workbench-bar-views" data-tour="view-switcher">
        ...
        {(hasFiles || isCustomView) && viewOptions.length > 1 && (
