 ...
 *   <li>pdfelite's own config dir always rejected, even if an allowed root were misconfigured to
 *       contain it;
 *   <li>pdfelite-owned "implied" roots are always permitted (even with none configured): the local
...
public class FolderAccessGuard {
    ...
    public Path requirePermitted(Path dir) {
        ...
        for (Path protectedRoot : protectedRoots) {
            if (normalized.startsWith(protectedRoot)) {
                throw new IllegalArgumentException(
                        "folder may not point inside a protected pdfelite directory");
            }
        }
        // pdfelite-owned implied roots are always permitted, even with no configured roots, so
    ...
    /** The pdfelite-owned directories always permitted, with a reason key for each (read-only). */
    ...
    /**
     * pdfelite-owned directories always permitted regardless of {@code allowedFolderRoots}, so
