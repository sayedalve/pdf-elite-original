package pdfelite.software.saas.procurement.license;
...
import pdfelite.software.saas.procurement.config.KeygenConfigurationProperties;
 ...
 * the self-hosted checkout uses. Active only when {@code pdfelite.keygen.enabled=true}; otherwise
...
@Service
@Profile("saas")
@ConditionalOnProperty(name = "pdfelite.keygen.enabled", havingValue = "true")
    ...
    public KeygenEnterpriseLicenseService(KeygenConfigurationProperties config) {
        ...
        if (!config.isConfigured()) {
            throw new IllegalStateException(
                    "pdfelite.keygen.enabled=true but Keygen is not fully configured — set "
                            + "pdfelite_KEYGEN_ACCOUNT_ID / _API_TOKEN / _POLICY_ID, or turn the flag off");
        }
    }
