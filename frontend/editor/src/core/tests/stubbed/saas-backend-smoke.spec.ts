 ...
 *   PDFElite_FLAVOR=saas ./gradlew :pdf-elite:bootRun --args="--server.port=18083 --spring.profiles.include=dev"
 *   PDFElite_SAAS_URL=http://localhost:18083 npx playwright test --project=stubbed saas-backend-smoke
 */
import { test, expect, request } from "@playwright/test";

const SAAS_URL = process.env.PDFElite_SAAS_URL ?? "http://localhost:18083";
...
test.describe("SaaS backend smoke", () => {
  ...
  test("CORS preflight rejects tenant wildcard origin (regression #11)", async () => {
    ...
    const r = await ctx.fetch(`${SAAS_URL}/api/v1/credits`, {
      method: "OPTIONS",
      headers: {
        Origin: "https://abandoned-tenant.ssl.PDFElitepdf.cloud",
        "Access-Control-Request-Method": "GET",
      },
    });
    expect(r.status()).toBe(403);
