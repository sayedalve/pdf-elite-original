package pdfelite.software.saas.payg.filter;
...
import pdfelite.software.common.util.TempFileRegistry;
...
import pdfelite.software.saas.payg.charge.ChargeOutcome;
import pdfelite.software.saas.payg.charge.JobChargeService;
import pdfelite.software.saas.payg.job.JobService;
import pdfelite.software.saas.payg.model.JobStepStatus;
...
class PaygChargeInterceptorTest {
    ...
    void preHandle_desktopClientHeader_setsJobSourceDesktopApp() throws Exception {
        ...
        when(chargeService.openProcess(any(), anyList()))
                .thenReturn(new ChargeOutcome(jobId, 1, ChargeOutcome.Disposition.OPENED));
        org.mockito.ArgumentCaptor<pdfelite.software.saas.payg.charge.ChargeContext> ctxCaptor =
                org.mockito.ArgumentCaptor.forClass(
                        pdfelite.software.saas.payg.charge.ChargeContext.class);

        MockMultipartHttpServletRequest req = newMultipart();
        req.addFile(new MockMultipartFile("file", "x.pdf", "application/pdf", "abc".getBytes()));
        req.addHeader("X-pdfelite-Client", "desktop");
        ...
        assertThat(ctxCaptor.getValue().source())
                .isEqualTo(pdfelite.software.saas.payg.model.JobSource.DESKTOP_APP);
    }
    ...
    void preHandle_pipelineHeader_setsJobSourcePipeline() throws Exception {
        ...
        MockMultipartHttpServletRequest req = newMultipart();
        req.addFile(new MockMultipartFile("file", "x.pdf", "application/pdf", "abc".getBytes()));
        req.addHeader("X-pdfelite-Automation", "true");

        interceptor.preHandle(req, new MockHttpServletResponse(), handlerMethodForFakeController());

        verify(chargeService).openProcess(ctxCaptor.capture(), anyList());
        assertThat(ctxCaptor.getValue().source())
                .isEqualTo(pdfelite.software.saas.payg.model.JobSource.PIPELINE);
    }

    @Test
    void preHandle_runId_honouredOnlyWithAutomationHeader() throws Exception {
        // An internal dispatch carries BOTH X-pdfelite-Automation and X-pdfelite-Run-Id, so the
        ...
        req.addFile(new MockMultipartFile("file", "x.pdf", "application/pdf", "abc".getBytes()));
        req.addHeader("X-pdfelite-Automation", "true");
        req.addHeader("X-pdfelite-Run-Id", "run-abc");
    ...
    void preHandle_runIdWithoutAutomationHeader_isIgnored() throws Exception {
        // A raw external API call that sets X-pdfelite-Run-Id on its own must NOT be able to group
        ...
        MockMultipartHttpServletRequest req = newMultipart();
        req.addFile(new MockMultipartFile("file", "x.pdf", "application/pdf", "abc".getBytes()));
        req.addHeader("X-pdfelite-Run-Id", "run-spoofed");
    ...
    void preHandle_apiKeyAuth_setsBillingCategoryApi() throws Exception {
        ...
        assertThat(ctxCaptor.getValue().billingCategory())
                .isEqualTo(pdfelite.software.saas.payg.model.BillingCategory.API);
    }
    ...
    void preHandle_requiresFeatureAutomation_setsBillingCategoryAutomation() throws Exception {
        ...
        verify(chargeService).openProcess(ctxCaptor.capture(), anyList());
        assertThat(ctxCaptor.getValue().billingCategory())
                .isEqualTo(pdfelite.software.saas.payg.model.BillingCategory.AUTOMATION);
    }
    ...
    void preHandle_requiresFeatureAiSupport_setsBillingCategoryAi() throws Exception {
        ...
        verify(chargeService).openProcess(ctxCaptor.capture(), anyList());
        assertThat(ctxCaptor.getValue().billingCategory())
                .isEqualTo(pdfelite.software.saas.payg.model.BillingCategory.AI);
    }
    ...
    void preHandle_aiEndpointWithAutomationHeader_automationWinsByPrecedence() throws Exception {
        // X-pdfelite-Automation: true on an @RequiresFeature(AI_SUPPORT) endpoint → AUTOMATION
    ...
    void preHandle_aiToolRoute_withAutomationHeader_isAutomation() throws Exception {
        // An AI tool dispatched inside a policy / AI workflow carries X-pdfelite-Automation: true →
    ...
    private static User makeUser(Long id, Long teamId) {
        ...
        if (teamId != null) {
            pdfelite.software.proprietary.model.Team team =
                    new pdfelite.software.proprietary.model.Team();
            team.setId(teamId);
            user.setTeam(team);
        }
        return user;
    ...
    private void authenticateWithApiKey(User user) {
        pdfelite.software.proprietary.security.model.ApiKeyAuthenticationToken token =
                new pdfelite.software.proprietary.security.model.ApiKeyAuthenticationToken(
    ...
    static class FakeController {
        ...
        @pdfelite.software.saas.payg.cap.RequiresFeature(
                pdfelite.software.saas.payg.model.FeatureGate.AUTOMATION)
        public void handleAutomation() {}

        @AutoJobPostMapping(value = "/ai", resourceWeight = 1)
        @pdfelite.software.saas.payg.cap.RequiresFeature(
                pdfelite.software.saas.payg.model.FeatureGate.AI_SUPPORT)
