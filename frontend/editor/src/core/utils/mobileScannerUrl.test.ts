...
describe("buildMobileScannerUrl", () => {
  test("origin-only frontendUrl keeps the app base path (SaaS web regression)", () => {
    expect(
      buildMobileScannerUrl({
        configuredUrl: "https://app.PDFElitepdf.com",
        sessionId,
        origin: "https://app.PDFElitepdf.com",
        basePath: "/app",
      }),
    ).toBe("https://app.PDFElitepdf.com/app/mobile-scanner?session=abc-123");
  });
  ...
  test("configured URL that already carries the subpath is used verbatim (no doubled base)", () => {
    expect(
      buildMobileScannerUrl({
        configuredUrl: "https://app.PDFElitepdf.com/app",
        sessionId,
