 ...
 * writes (the `PDFElitePDFClassification` Info-dict key). Loading PDF bytes is
...
export const CLASSIFICATION_METADATA_KEY = "PDFElitePDFClassification";
...
export async function readStubClassificationLabels(
  ...
  const file = await fileStorage
    .getPDFEliteFile(stub.id as FileId)
    .catch(() => null);
  if (!file) return null;
