...
export function WorkbenchBarProvider({
    ...
    (newButtons: WorkbenchBarButtonConfig[]) => {
      setButtons((prev) => {
        ...
        if (process.env.NODE_ENV === "development") {
          ...
          if (dupes.length)
            console.warn(
              "[WorkbenchBar] Duplicate ids in registerButtons:",
              dupes,
            );
        }
        return merged;
  ...
  const setAllButtonsDisabled = useCallback((disabled: boolean) => {
    setAllButtonsDisabledState(disabled);
  }, []);
...
export function useWorkbenchBar() {
  ...
  if (!ctx)
    throw new Error("useWorkbenchBar must be used within WorkbenchBarProvider");
  return ctx;
}
