...
import { PDFElite_SAAS_URL } from "@app/constants/connection";
...
export const SetupWizard: React.FC<SetupWizardProps> = ({
    ...
    const unsubscribePromise = listen<string>("deep-link", async (event) => {
      ...
      try {
        ...
        if (type === "sso" || type === "sso-selfhosted") {
          ...
          const serverUrl =
            serverFromQuery || serverConfig?.url || PDFElite_SAAS_URL;
          if (!token || !serverUrl) {
            console.error(
        ...
        await authService.completeSupabaseSession(
          accessToken,
          serverConfig?.url || PDFElite_SAAS_URL,
        );
  ...
  const handleBack = () => {
    ...
    if (activeStep === SetupStep.SelfHostedLogin) {
      ...
      setActiveStep(SetupStep.SaaSLogin);
      setServerConfig({ url: PDFElite_SAAS_URL });
  ...
  const wizardContent = (
    <>
      ...
      {!lockConnectionMode && activeStep === SetupStep.SaaSLogin && (
        <SaaSLoginScreen
          serverUrl={serverConfig?.url || PDFElite_SAAS_URL}
