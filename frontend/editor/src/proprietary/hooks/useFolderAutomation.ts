 ...
 * - PDFEliteFiles (already in fileStorage): returns the existing fileId, ownedByFolder=false.
...
export async function resolveInputFile(
  ...
  await fileStorage.storePDFEliteFile(
    createPDFEliteFile(file, newFileId),
    stub,
  );
...
async function finalizeRun(
  ...
  const inputStub = await fileStorage.getPDFEliteFileStub(
  ...
  if (isAutoNumber) {
    for (const meta of Object.values(currentFolderData?.files ?? {})) {
      ...
      for (const oid of ids) {
        const stub = await fileStorage.getPDFEliteFileStub(oid as FileId);
  ...
  for (const resultFile of resultFiles) {
    ...
    const outputStub: PDFEliteFileStub = {
      id: outputId,
      name: outputFileName,
    ...
    const renamedFile =
      outputFileName !== resultFile.name
        ...
        : resultFile;
    await fileStorage.storePDFEliteFile(
      createPDFEliteFile(renamedFile, outputId),
      outputStub,
    );
  ...
  if (!isAutoNumber) {
    for (const oldId of prevOutputIds) {
      try {
        await fileStorage.deletePDFEliteFile(oldId as FileId);
      } catch {
        /* ignore */
      }
    }
  ...
  useEffect(() => {
    async function drainDueRetries() {
      ...
      for (const entry of due) {
        ...
        const freshFile = await fileStorage.getPDFEliteFile(
