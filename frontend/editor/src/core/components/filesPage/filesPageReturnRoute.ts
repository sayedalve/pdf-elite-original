...
const SESSION_KEY = "PDFElite.filesPage.returnRoute";
const SESSION_LABEL_KEY = "PDFElite.filesPage.returnLabel";
...
export function setFilesPageReturnRoute(route: string, label?: string): void {
  try {
    ...
    refreshSnapshot();
    window.dispatchEvent(new CustomEvent("PDFElite-filespage-return-changed"));
  } catch {
    /* ignore */
  }
}
...
export function subscribeFilesPageReturnRoute(
  ...
  window.addEventListener("PDFElite-filespage-return-changed", handler);
  return () => {
    window.removeEventListener("storage", handler);
    window.removeEventListener("PDFElite-filespage-return-changed", handler);
  };
}
