#!/bin/bash
# aot-diagnostics.sh - Project Leyden AOT cache diagnostic tool for pdf-elite
#
...
#   --cache PATH   Override the AOT cache path (default: /configs/cache/PDFElite.aot)
#
# Symlink aliases set up by init-without-ocr.sh: aot-diag, aot-diagnostics

set -euo pipefail

AOT_CACHE_DEFAULT="/configs/cache/PDFElite.aot"
...
info "Kernel:       $(uname -r)"
if [ -f /etc/PDFElite_version ]; then
  info "Version:      $(tr -d '\r\n' < /etc/PDFElite_version)"
elif [ -n "${VERSION_TAG:-}" ]; then
  info "Version:      ${VERSION_TAG}"
else
  warn "VERSION_TAG not set and /etc/PDFElite_version not found"
fi
...
if [ -f /app/app.jar ] && [ -d /app/lib ]; then
  ...
  info "  Classpath: -cp /app/app.jar:/app/lib/* PDFElite.software.SPDF.SPDFApplication"
  JAR_LAYOUT="layered"
elif [ -f /app.jar ]; then
...
if [ "$RUN_SMOKE_TEST" = true ]; then
  ...
  if [ "$AOT_SUPPORTED" = false ]; then
    ...
    SMOKE_CMD=(java -Xmx256m ${COMPACT_HEADERS_FLAG:-} ${COMPRESSED_OOPS_FLAG}
      ...
      -DPDFElite.datasource.url="jdbc:h2:mem:aotsmoke;DB_CLOSE_DELAY=-1;MODE=PostgreSQL")

    case "$JAR_LAYOUT" in
      layered)  SMOKE_CMD+=(-cp "/app/app.jar:/app/lib/*" PDFElite.software.SPDF.SPDFApplication) ;;
...
if [ "$FAIL" -gt 0 ]; then
  ...
  printf "To disable AOT: omit PDFElite_AOT_ENABLE (default is off) or set PDFElite_AOT_ENABLE=false\n"
  exit 1
elif [ "$WARN" -gt 0 ]; then
