...
function describeStandardOperation<TParams>(
  ...
  let defaultParameters = (config.defaultParameters || {}) as TParams;
  if (isRedact) {
    defaultParameters = {
      ...defaultParameters,
      mode: "automatic",
...
export const POLICY_OPERATIONS = {
  redact: describeStandardOperation(
    "/api/v1/security/auto-redact",
    redactOperationConfig,
  ),
  sanitize: describeStandardOperation(
  ...
  timestampPdf: describeStandardOperation(
    "/api/v1/security/timestamp-pdf",
    timestampPdfOperationConfig,
  ),
  watermark: describeStandardOperation(
    "/api/v1/security/add-watermark",
    addWatermarkOperationConfig,
  ),
  ocr: describeStandardOperation("/api/v1/misc/ocr-pdf", ocrOperationConfig),
  flatten: describeStandardOperation(
    "/api/v1/misc/flatten",
    flattenOperationConfig,
  ),
  compress: describeStandardOperation(
...
export function policyEndpoint(toolId: PolicyToolId): PolicyEndpoint {
  return POLICY_OPERATIONS[toolId].endpoint as PolicyEndpoint;
}
