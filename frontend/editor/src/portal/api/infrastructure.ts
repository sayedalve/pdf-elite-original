...
export interface DeploymentRegion {
  ...
  load: number;
  status: RegionStatus;
  /** Deployed PDFElite engine version. */
  version: string;
...
export type AccessPolicy = "PDFElite" | "byok" | "hyok";
 ...
 *   - `managed` — PDFElite-owned KMS keys; zero key ops on the customer side.
 *   - `byok` — customer key, but PDFElite can use it to decrypt while processing.
 *   - `hyok` — key never leaves the customer KMS; PDFElite holds only ciphertext.
 */
export type KeyMode = "managed" | "byok" | "hyok";

export interface KeyManagement {
  mode: KeyMode;
  /** Human-readable provider, e.g. "PDFElite KMS" or "AWS KMS (customer)". */
  provider: string;
  ...
  /**
   * Whether the customer may switch key custody (BYOK/HYOK). PDFElite-managed
...
export type ModelProvider = "PDFElite" | "openai" | "anthropic" | "on-prem";
