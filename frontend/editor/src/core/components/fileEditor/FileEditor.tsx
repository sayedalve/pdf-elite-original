...
interface FileEditorProps {
  ...
  onMergeFiles?: (files: PDFEliteFile[]) => void;
  toolMode?: boolean;
...
const FileEditor = ({
  ...
  const activePDFEliteFileStubs = useMemo(
    () => selectors.getPDFEliteFileStubs(),
  ...
  const stubsRef = useRef(activePDFEliteFileStubs);
  stubsRef.current = activePDFEliteFileStubs;
    ...
    async (fileId: FileId) => {
      ...
      if (record && file) {
        ...
        if (result.savedPath) {
          ...
          fileActions.updatePDFEliteFileStub(fileId, {
      ...
      if (record && file) {
        try {
          ...
          if (result.success && result.extractedStubs.length > 0) {
            ...
            await fileActions.addPDFEliteFileStubs(result.extractedStubs);
  ...
  return (
    ...
    >
      <Box pos="relative" style={{ overflow: "auto" }}>
        ...
        <Box p="md">
          {activePDFEliteFileStubs.length === 0 ? (
            <Center h="60vh">
            ...
            >
              ...
              {activePDFEliteFileStubs.map((record, index) => {
                return (
                  <FileEditorThumbnail
                    key={record.id}
                    file={record}
                    index={index}
                    totalFiles={activePDFEliteFileStubs.length}
