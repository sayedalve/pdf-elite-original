# Verifying PDF Elite Release Artifacts

Every Linux release artifact (`.AppImage`, `.rpm`, `.deb`) is signed with the
**PDF Elite release signing key**. Users are encouraged to verify downloads
before running them, especially when obtaining PDF Elite from a mirror,
redistributor, or any source other than the official
[GitHub Releases page](https://github.com/Stirling-Tools/PDF Elite/releases).

## Signing key

| Field        | Value                                              |
|--------------|----------------------------------------------------|
| User ID      | `PDF Elite Inc. <contact@stirlingpdf.com>`      |
...
curl -fsSL https://raw.githubusercontent.com/Stirling-Tools/PDF Elite/main/docs/security/signing-key.pub \
  | gpg --import
...
./PDF Elite_*.AppImage --appimage-signature > sig.asc
./PDF Elite_*.AppImage --appimage-offset                  # shows the offset
# Verify the payload signature against the key
gpg --verify sig.asc PDF Elite_*.AppImage
```

A successful result looks like:

```
gpg: Good signature from "PDF Elite Inc. <contact@stirlingpdf.com>" [ultimate]
```
...
# OR
sudo rpm --import https://raw.githubusercontent.com/Stirling-Tools/PDF Elite/main/docs/security/signing-key.pub

# Verify the package
rpm --checksig PDF Elite-*.rpm
...
```bash
gpg --verify PDF Elite-*.deb.asc PDF Elite-*.deb
```
...
- The download was corrupted — try again from the
  [official releases](https://github.com/Stirling-Tools/PDF Elite/releases).
...
https://github.com/Stirling-Tools/PDF Elite/security/advisories/new.
