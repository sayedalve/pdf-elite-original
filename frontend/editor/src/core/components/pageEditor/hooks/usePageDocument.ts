...
export function usePageDocument(): PageDocumentHook {
  ...
  const activeFileIds = useMemo(() => {
    return allFileIds.filter((id) => {
      const stub = selectors.getPDFEliteFileStub(id);
  ...
  const primaryPDFEliteFileStub = primaryFileId
    ? selectors.getPDFEliteFileStub(primaryFileId)
    : null;
  const processedFilePages = primaryPDFEliteFileStub?.processedFile?.pages;
  const processedFileTotalPages =
    primaryPDFEliteFileStub?.processedFile?.totalPages;
  ...
  useEffect(() => {
    ...
    if (primaryPDFEliteFileStub?.processedFile) {
      placeholderDocumentRef.current = null;
      setPlaceholderVersion((v) => v + 1);
      return;
    }
      ...
      try {
        ...
        if (!canceled) {
          placeholderDocumentRef.current = {
            ...
            name:
              selectors.getPDFEliteFileStub(primaryFileId)?.name ?? file.name,
            file,
            pages,
            totalPages,
          };
  ...
  }, [primaryFileId, primaryPDFEliteFileStub?.processedFile, selectors]);
  ...
  const mergedPdfDocument = useMemo((): PDFDocument | null => {
    ...
    if (
      !primaryPDFEliteFileStub?.processedFile &&
      placeholderDocumentRef.current
    ) {
    ...
    const name =
      namingFileIds.length <= 1
        ? namingFileIds[0]
          ? (selectors.getPDFEliteFileStub(namingFileIds[0])?.name ??
            "document.pdf")
          : "document.pdf"
        : namingFileIds
            .map((id) =>
              (selectors.getPDFEliteFileStub(id)?.name ?? "file").replace(
                /\.pdf$/i,
                "",
              ),
            )
    ...
    activeFileIds.forEach((fileId) => {
      const record = selectors.getPDFEliteFileStub(fileId);
    ...
    const createPagesFromFile = (
      ...
      const PDFEliteFileStub = selectors.getPDFEliteFileStub(fileId);
      if (!PDFEliteFileStub) {
        return [];
      }
      ...
      const processedFile = PDFEliteFileStub.processedFile;
