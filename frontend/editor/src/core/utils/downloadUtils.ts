...
export async function downloadFileFromStorage(
  ...
  const PDFEliteFile = await fileStorage.getPDFEliteFile(lookupKey);
...
export async function downloadFilesAsZip(
  ...
  for (const fileWithUrl of files) {
    const PDFEliteFile = await fileStorage.getPDFEliteFile(fileWithUrl.id);
    if (PDFEliteFile) {
      filesToZip.push(PDFEliteFile);
