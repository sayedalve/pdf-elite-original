...
export const usePageEditorExport = ({
  ...
  const applyChanges = useCallback(async () => {
    ...
    try {
      ...
      const newPDFEliteFiles = await actions.addFiles(renamedFiles, {
        selectFiles: true,
        skipUploadTracking: true,
      });
      if (newPDFEliteFiles.length > 0) {
        actions.setSelectedFiles(newPDFEliteFiles.map((file) => file.fileId));
      }

      if (sourceFileIds.length === 1 && newPDFEliteFiles.length === 1) {
        const sourceStub = selectors.getPDFEliteFileStub(sourceFileIds[0]);
        if (sourceStub?.localFilePath) {
          actions.updatePDFEliteFileStub(newPDFEliteFiles[0].fileId, {
