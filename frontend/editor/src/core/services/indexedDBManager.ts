...
class IndexedDBManager {
  ...
  async openDatabase(config: DatabaseConfig): Promise<IDBDatabase> {
    ...
    // SaaS lineage shipped a v6 and a v7 of pdf-elite-files whose
    ...
    if (config.name === "pdf-elite-files") {
  ...
  /**
   * Single-pass migration for the `files` store on pdf-elite-files.
   *
