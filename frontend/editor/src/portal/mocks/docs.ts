...
const QUICKSTART_SAMPLES: CodeSample[] = [
  {
    ...
    label: "cURL",
    lang: "bash",
    code: `curl https://api.PDFElitepdf.com/v1/invoice \\
  -H "Authorization: Bearer $PDFElite_API_KEY" \\
  -F "file=@invoice.pdf"`,
  },
  {
    key: "python",
    label: "Python",
    lang: "python",
    code: `from PDFElite import PDFElite

client = PDFElite(api_key="sk_live_...")
...
print(result.fields["total"])`,
  },
  {
    ...
    label: "Node",
    lang: "typescript",
    code: `import { PDFElite } from "@PDFElite/sdk";

const client = new PDFElite({ apiKey: process.env.PDFElite_API_KEY });
...
const SDKS: Sdk[] = [
  ...
  },
  {
    ...
    icon: "⬢",
    install: "npm install @PDFElite/sdk",
    lang: "bash",
    status: "ga",
  },
  {
    name: "Go",
    icon: "◉",
    install: "go get github.com/PDFElite/PDFElite-go",
    lang: "bash",
    status: "ga",
  },
  {
  ...
  },
  {
    name: "Java",
    icon: "☕",
    install: "implementation 'com.PDFElite:sdk:1.x'",
    lang: "bash",
    status: "ga",
  },
  {
    name: ".NET",
    icon: "◈",
    install: "dotnet add package PDFElite",
    lang: "bash",
    status: "beta",
  },
  // PHP client predates the typed-response rewrite; pinned, no new endpoints.
  {
    name: "PHP",
    icon: "🐘",
    install: "composer require PDFElite/PDFElite-php:^0.9",
    lang: "bash",
    status: "deprecated",
  },
];
...
const COMPONENTS: EmbedComponent[] = [
  ...
  {
    name: "Web component",
    blurb: "<PDFElite-viewer> custom element for non-React stacks.",
    tag: "Web",
  },
];
