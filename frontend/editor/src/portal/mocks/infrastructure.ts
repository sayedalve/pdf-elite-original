...
const CERTS_FREE: ComplianceCert[] = [
  {
    ...
    name: "SOC 2 Type II",
    status: "certified",
    detail: "Inherited — PDFElite platform",
  },
  {
    id: "iso",
    name: "ISO 27001",
...
// PDFElite-managed key custody — what free/pro tiers run on. No key ops on the
// customer side, so the provider/key fields describe PDFElite's own KMS.
const KEY_MANAGED: KeyManagement = {
  mode: "managed",
  provider: "PDFElite KMS",
  keyId: "arn:PDFElite:kms:us-east-1:platform/cmk-default",
...
export function securityFor(tier: Tier): SecurityConfig {
  ...
  if (tier === "pro") {
    return {
      ...
      // Pro stays on PDFElite-managed keys; BYOK/HYOK is an enterprise lever.
      keyManagement: KEY_MANAGED,
      attestations: ATTESTATIONS_FREE,
    };
  }
...
export function modelsFor(tier: Tier): ModelEntry[] {
  // Free sees only the two managed PDFElite models it can actually use.
  if (tier === "free")
