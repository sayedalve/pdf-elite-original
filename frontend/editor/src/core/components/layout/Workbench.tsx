...
export default function Workbench() {
  ...
  const { loadMetadata } = useIndexedDB();

  const handleOpenDoc = async (doc: any) => {
    try {
      const stub = await loadMetadata(doc.id);
      if (stub) {
        await fileActions.addPDFEliteFileStubs([stub], { selectFiles: true });
  ...
  const handleCreateBlankPdf = async () => {
    try {
      ...
      // Add a standard A4 page (595.28 x 841.89)
      ...
      case "fileEditor":
        return (
          ...
          >
            ...
            {/* Real registry-tool host. The reskin previously never mounted the
                tool component (RightSidebar → ToolPanel → ToolRenderer was
                orphaned), so file-level tools such as Merge showed as dead
                cards. Rendering the existing ToolRenderer here reconnects the
                real tool UI (file sort, settings, execute) beside the file grid
                that drives selection/reordering. */}
            {selectedToolId && selectedTool?.component && (
              <aside
                style={{
                  flex: "0 0 400px",
                  width: 400,
                  ...
                  background: "var(--surface-elevated, var(--c-bg))",
                }}
              >
                <ToolRenderer
