...
import pdfelite.software.saas.service.SaasUserAccountService;
...
class UserRoleWebhookControllerTest {
    ...
    class PromptToAuthUser {
        ...
        void happyPath() {
            ...
            SupabaseUser supabaseUser = supabaseUserWithEmail("new@pdfelite.com");
            when(supabaseUserService.getUser(LINKED_SUPABASE_ID)).thenReturn(supabaseUser);

            User upgraded = new User();
            upgraded.setId(42L);
            upgraded.setEmail("new@pdfelite.com");
            upgraded.setUsername("new@pdfelite.com");
            when(saasUserAccountService.synchronizeUserUpgrade(
                            supabaseUser, "new@pdfelite.com", "google"))
                    .thenReturn(upgraded);
            ...
            assertThat(response.getBody())
                    ...
                    .containsEntry("email", "new@pdfelite.com");
        }
        ...
        void canonicalEmailFallsBackToLocal() {
            User current = anonymousUser(LINKED_SUPABASE_ID);
            current.setEmail("local@pdfelite.com");
            when(userService.findByUsername(USERNAME)).thenReturn(Optional.of(current));
            ...
            User upgraded = new User();
            upgraded.setId(5L);
            upgraded.setEmail("local@pdfelite.com");
            when(saasUserAccountService.synchronizeUserUpgrade(
                            eq(supabaseUser), eq("local@pdfelite.com"), anyString()))
                    .thenReturn(upgraded);
            ...
            assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
            verify(saasUserAccountService)
                    .synchronizeUserUpgrade(supabaseUser, "local@pdfelite.com", "email");
        }
