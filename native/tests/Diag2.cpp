#include <iostream>
#include <fpdfview.h>
#include <fpdf_edit.h>
#include <fpdf_text.h>
#include <vector>
#include <string>

int main() {
    FPDF_InitLibrary();
    FPDF_DOCUMENT doc = FPDF_LoadDocument("basic_text.pdf", nullptr);
    if (!doc) {
        std::cout << "Doc load failed" << std::endl;
        return 1;
    }
    FPDF_PAGE page = FPDF_LoadPage(doc, 0);
    
    FPDF_PAGEOBJECT textObj = FPDFPageObj_NewTextObj(doc, "Arial", 14.0f);
    FPDFText_SetText(textObj, (FPDF_WIDESTRING)L"New Added Text");
    FPDFPage_InsertObject(page, textObj);
    FPDFPage_GenerateContent(page);
    
    FPDF_TEXTPAGE textPage = FPDFText_LoadPage(page);
    unsigned long len = FPDFTextObj_GetText(textObj, textPage, nullptr, 0);
    std::cout << "Len: " << len << std::endl;
    if (len > 0) {
        std::vector<FPDF_WCHAR> buf(len/2);
        FPDFTextObj_GetText(textObj, textPage, buf.data(), len);
        std::wstring str(buf.begin(), buf.end() - 1);
        std::wcout << L"Text: " << str << std::endl;
    } else {
        std::cout << "Len was 0" << std::endl;
    }
    
    FPDFText_ClosePage(textPage);
    FPDF_ClosePage(page);
    FPDF_CloseDocument(doc);
    FPDF_DestroyLibrary();
    return 0;
}
