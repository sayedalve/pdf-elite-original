...
public class McpSecurityConfig {
    ...
    /**
     * API-key chain: a pdfelite per-user API key is validated by {@link McpApiKeyAuthFilter};
     * otherwise 401.
     */
                        ...
                        e ->
                                e.authenticationEntryPoint(
                                        (request, response, ex) -> {
                                            ...
                                            response.setHeader(
                                                    "WWW-Authenticate",
                                                    "Bearer realm=\"pdfelite MCP (API key)\"");
                                            response.setContentType("application/json");
                                            response.getWriter()
                                                    .write(
                                                            "{\"error\":\"unauthorized\",\"message\":\"Provide a valid pdfelite API key via the X-API-KEY header (or Authorization: Bearer <key>).\"}");
                                        }))
                .addFilterBefore(
                ...
                .authorizeHttpRequests(
                        a ->
                                a.requestMatchers(
                                        ...
                                        .anyRequest()
                                        .authenticated())
                // Cap body size pre-auth, then bind the validated token to a pdfelite user after
                // the bearer filter.
                .addFilterBefore(
                        new McpRequestSizeFilter(
                                applicationProperties.getMcp().getMaxRequestBytes()),
