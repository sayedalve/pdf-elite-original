...
export async function reconcileServerFiles(
  ...
  let combinedStubs: PDFEliteFileStub[];
 ...
 * `updateStub` (from FileContext.updatePDFEliteFileStub) so this util
...
export async function materializeServerStubs(
  ...
  helpers: {
    ...
    updateStub: (id: FileId, updates: Partial<PDFEliteFileStub>) => void;
  },
): Promise<PDFEliteFileStub[]> {
  const out: PDFEliteFileStub[] = [];
