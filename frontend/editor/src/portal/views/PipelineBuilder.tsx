...
export function PipelineBuilder() {
  ...
  useEffect(() => {
    ...
    setSteps(
      (policy?.steps ?? []).map((step) => {
        const tool = executableTools.find(
          (t) =>
            t.operation === step.operation ||
            t.endpoints?.includes(step.operation),
        );
        return deserializeToolStep({
          toolId: tool ? tool.id : null,
          operation: step.operation,
          parameters: step.parameters,
        } as SerializedToolStep);
      }),
    );
  ...
  function addStep(tool: ExecutableTool) {
    setSteps((current) => {
      const next = [...current, newWorkingToolStep(tool)];
      setSelectedIndex(next.length - 1);
      return next;
    });
    setPickerOpen(false);
  ...
  const snapshot = JSON.stringify({
    ...
    enabled,
    input,
    steps: steps.map((step) => serializeToolStep(step)),
    uploads: steps.map((step) => stepRequiresUpload(step)),
