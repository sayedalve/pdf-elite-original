...
export function usePolicyAutoRun(): void {
  ...
  const { addFiles, updatePDFEliteFileStub } = useFileManagement();
...
interface ImportContext {
  ...
  consumeFiles: (
    inputFileIds: FileId[],
    outputs: PDFEliteFile[],
    stubs: PDFEliteFileStub[],
  ...
  parentStub: PDFEliteFileStub | undefined;
...
async function importOutputs(
  ...
  const parentStub =
    ctx.outputMode === "new_version"
      ? (ctx.parentStub ??
        (await fileStorage.getPDFEliteFileStub(run.fileId as FileId)) ??
        undefined)
      : undefined;
  ...
  if (parentStub) {
    ...
    await Promise.all(
      added.map(async (f, i) => {
        ...
        ctx.updatePDFEliteFileStub(f.fileId, updates);
...
async function runPolicyOnFile(
  ...
  // its stub can appear in the file list a beat before getPDFEliteFile resolves
  ...
  const tryGetFile = async (): Promise<PDFEliteFile | null> => {
    try {
      return await fileStorage.getPDFEliteFile(fileId);
    } catch {
      return null;
    }
  };
