...
export const TourOrchestrationProvider: React.FC<{
  ...
  const savedFilesRef = useRef<PDFEliteFile[]>([]);

  // Keep a ref to always have the latest files
  const filesRef = useRef<PDFEliteFile[]>(files);
  React.useEffect(() => {
  ...
  const restoreWorkbenchState = useCallback(async () => {
      ...
      try {
        await Promise.all(
          currentFiles.map((file) =>
            fileStorage.deletePDFEliteFile(file.fileId),
          ),
        );
    ...
    if (savedFilesRef.current.length > 0) {
      // Create fresh File objects from PDFEliteFile to avoid ID conflicts
