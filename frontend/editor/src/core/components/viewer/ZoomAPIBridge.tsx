...
function ZoomAPIBridgeInner({ documentId }: { documentId: string }) {
  ...
  const stubs = selectors.getPDFEliteFileStubs();
  ...
  useEffect(() => {
    ...
    const applyTrackedZoom = (
      ...
      triggerImmediateZoomUpdate(Math.round(effectiveZoom * 75));
    ...
    const applyAutoZoom = async () => {
      ...
      // 1. Check if we have a saved zoom level for this specific file
      if (firstFileId) {
        const savedZoom = pageMemoryService.getZoom(firstFileId);
        if (savedZoom !== undefined) {
          applyTrackedZoom(savedZoom, savedZoom);
          return;
        }
      }

      // 2. Fall back to user preference for default viewer zoom
      ...
      if (zoomPref !== "auto") {
        if (zoomPref === "fitWidth") {
          ...
          const numericZoom = parseInt(zoomPref, 10) / 75;
  ...
  useEffect(() => {
    ...
    zoomSubscriptionRef.current = zoom.onZoomChange(
      (event: { newZoom?: number }) => {
        ...
        triggerImmediateZoomUpdate(Math.round(event.newZoom * 75));
        if (firstFileId) {
          pageMemoryService.saveZoom(firstFileId, event.newZoom);
        }
      },
    );
  ...
  }, [zoom, triggerImmediateZoomUpdate, firstFileId]);
  ...
  useEffect(() => {
    ...
    const newState = {
      ...
      zoomPercent: Math.round(currentZoomLevel * 75),
    };
