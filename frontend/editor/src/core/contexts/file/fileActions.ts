...
/**
 * Create a child PDFEliteFileStub from a parent stub with proper history management.
 * Used when a tool processes an existing file to create a new version with incremented history.
 *
 * @param parentStub - The parent PDFEliteFileStub to create a child from
 ...
 * @returns New child PDFEliteFileStub with proper version history
 */
...
): Promise<PDFEliteFile[]> {
  // Acquire mutex to prevent race conditions
  await addFilesMutex.lock();

  try {
    const PDFEliteFileStubs: PDFEliteFileStub[] = [];
    ...
    // Collect hydrations to schedule after dispatch so updatePDFEliteFileStub finds files in state.
    ...
    const flushChunk = () => {
      if (
        ...
        PDFEliteFileStubs.length > flushedStubs
      ) {
        dispatch({
          type: "ADD_FILES",
          payload: { PDFEliteFileStubs: PDFEliteFileStubs.slice(flushedStubs) },
        });
        flushedStubs = PDFEliteFileStubs.length;
      }
      // Hydrations only after their chunk is dispatched, so
      // updatePDFEliteFileStub finds the files in state.
    ...
    for (const file of filesToProcess) {
      ...
      const fileStub = createNewPDFEliteFileStub(file, fileId);
      ...
      PDFEliteFileStubs.push(fileStub);

      // Create PDFEliteFile directly
      const PDFEliteFile = createPDFEliteFile(file, fileId);
      ...
      pendingHydrations.push(async () => {
        ...
        const updates: Partial<PDFEliteFileStub> = {};
        ...
        if (Object.keys(updates).length > 0) {
          lifecycleManager.updatePDFEliteFileStub(fileId, updates, stateRef);
        }
      ...
      reportBulkAddProgress(++scannedCount, filesToProcess.length);
      if (PDFEliteFileStubs.length - flushedStubs >= DISPATCH_CHUNK) {
        flushChunk();
      }
    }
    ...
    if (enablePersistence && PDFEliteFiles.length > 0) {
      await Promise.all(
        PDFEliteFiles.map(async (PDFEliteFile, index) => {
          try {
            // Get corresponding stub with all metadata
            const fileStub = PDFEliteFileStubs[index];

            // Store using the cleaner signature - pass PDFEliteFile + PDFEliteFileStub directly
            await fileStorage.storePDFEliteFile(PDFEliteFile, fileStub);

            if (DEBUG)
              console.log(
                `📄 addFiles: Stored file ${PDFEliteFile.name} with metadata:`,
                fileStub,
              );
          } catch (error) {
            console.error(
    ...
    if (!options.skipUploadTracking && PDFEliteFiles.length > 0) {
      trackPdfUploaded(PDFEliteFiles);
    }

    return PDFEliteFiles;
  } finally {
    clearBulkAddProgress();
 ...
 * Now accepts pre-created PDFEliteFiles and PDFEliteFileStubs to preserve all metadata
 */
export async function consumeFiles(
  inputFileIds: FileId[],
  outputPDFEliteFiles: PDFEliteFile[],
  outputPDFEliteFileStubs: PDFEliteFileStub[],
  filesRef: React.MutableRefObject<Map<FileId, File>>,
  dispatch: React.Dispatch<FileContextAction>,
  ...
  if (DEBUG)
    console.log(
      `📄 consumeFiles: Processing ${inputFileIds.length} input files, ${outputPDFEliteFiles.length} output files with pre-created stubs`,
    );

  // Validate that we have matching files and stubs
  if (outputPDFEliteFiles.length !== outputPDFEliteFileStubs.length) {
    throw new Error(
      `Mismatch between output files (${outputPDFEliteFiles.length}) and stubs (${outputPDFEliteFileStubs.length})`,
    );
  }

  // Store PDFEliteFiles in filesRef using their existing IDs (no ID generation needed)
  for (let i = 0; i < outputPDFEliteFiles.length; i++) {
    const PDFEliteFile = outputPDFEliteFiles[i];
    const stub = outputPDFEliteFileStubs[i];

    if (PDFEliteFile.fileId !== stub.id) {
      console.warn(
        `📄 consumeFiles: ID mismatch between PDFEliteFile (${PDFEliteFile.fileId}) and stub (${stub.id})`,
      );
    }

    filesRef.current.set(PDFEliteFile.fileId, PDFEliteFile);

    if (DEBUG)
      console.log(
        `📄 consumeFiles: Stored PDFEliteFile ${PDFEliteFile.name} with ID ${PDFEliteFile.fileId}`,
      );
  }
  ...
  dispatch({
    ...
    payload: {
      inputFileIds,
      outputPDFEliteFileStubs: outputPDFEliteFileStubs,
  ...
  if (DEBUG)
    console.log(
      `📄 consumeFiles: Successfully consumed files - removed ${inputFileIds.length} inputs, added ${outputPDFEliteFileStubs.length} outputs`,
    );
  // Return the output file IDs for undo tracking
  return outputPDFEliteFileStubs.map((stub) => stub.id);
}
...
export async function undoConsumeFiles(
  inputFiles: File[],
  inputPDFEliteFileStubs: PDFEliteFileStub[],
  ...
  if (DEBUG)
    console.log(
      `📄 undoConsumeFiles: Restoring ${inputPDFEliteFileStubs.length} input files, removing ${outputFileIds.length} output files`,
    );

  // Validate inputs
  if (inputFiles.length !== inputPDFEliteFileStubs.length) {
    throw new Error(
      `Mismatch between input files (${inputFiles.length}) and records (${inputPDFEliteFileStubs.length})`,
    );
  }
  ...
  try {
    ...
    inputFiles.forEach((file, index) => {
      const record = inputPDFEliteFileStubs[index];
    ...
    const stubsWithDirtyMarked = inputPDFEliteFileStubs.map((stub) =>
    ...
    dispatch({
      ...
      payload: {
        inputPDFEliteFileStubs: stubsWithDirtyMarked,
        outputFileIds,
      },
    });
    ...
    await Promise.all(
      inputPDFEliteFileStubs.map((stub) =>
    ...
    if (DEBUG)
      console.log(
        `📄 undoConsumeFiles: Successfully undone consume operation - restored ${inputPDFEliteFileStubs.length} inputs, removed ${outputFileIds.length} outputs`,
      );
  } catch (error) {
...
/**
 * Add files using existing PDFEliteFileStubs from storage - preserves all metadata
 * Use this when loading files that already exist in storage (FileManager, etc.)
 * PDFEliteFileStubs come with proper thumbnails, history, processing state
 */
export async function addPDFEliteFileStubs(
  ...
  try {
    ...
    for (const stub of PDFEliteFileStubs) {
      ...
      if (stateRef.current.files.byId[stub.id]) {
        if (DEBUG)
          console.log(
            `📄 Skipping already-loaded PDFEliteFileStub: ${stub.name}`,
          );
        continue;
      }
      ...
      dispatch({ type: "ADD_FILES", payload: { PDFEliteFileStubs: [record] } });
      ...
      scheduleMetadataHydration(async () => {
        const PDFEliteFile = await fileStorage.getPDFEliteFile(fileId);
        if (!PDFEliteFile) {
          return;
        }

        // Store the loaded file in filesRef
        filesRef.current.set(fileId, PDFEliteFile);

        // Check if processedFile data needs regeneration
        if (PDFEliteFile.type.startsWith("application/pdf")) {
          ...
          if (needsProcessing) {
            ...
            const processedFileMetadata =
              await generateProcessedFileMetadata(PDFEliteFile);
            ...
            if (processedFileMetadata) {
              ...
              lifecycleManager.updatePDFEliteFileStub(
                fileId,
                updates,
                stateRef,
              );
        ...
        lifecycleManager.updatePDFEliteFileStub(fileId, {}, stateRef);
      });
    }
...
export const createFileActions = (
  ...
  updatePDFEliteFileStub: (
    fileId: FileId,
    updates: Partial<PDFEliteFileStub>,
  ) =>
