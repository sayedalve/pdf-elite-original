...
# fresh working directory, boot the current pdf-elite JAR against it, then
...
#
# Inputs:
#   PDFElite_JAR  - path to a pre-built pdf-elite .jar (defaults to the
#                   :pdf-elite:bootJar output)
...
PDFElite_JAR="${PDFElite_JAR:-}"
JAVA_BIN="${JAVA_BIN:-${MIGRATION_TEST_JAVA:-}}"
ADMIN_USERNAME="${ADMIN_USERNAME:-admin}"
ADMIN_PASSWORD="${ADMIN_PASSWORD:-PDFElite}"
...
find_jar() {
    local candidate
    if [[ -n "$PDFElite_JAR" ]]; then
        [[ -f "$PDFElite_JAR" ]] || fail "PDFElite_JAR='$PDFElite_JAR' not found"
        candidate="$PDFElite_JAR"
    else
        candidate=$(find "$REPO_ROOT/app/core/build/libs" -maxdepth 1 -name 'pdf-elite*.jar' -o -name 'pdf-elite*.jar' 2>/dev/null \
            | grep -vE '(-plain|-sources)\.jar$' | head -n 1 || true)
        [[ -n "$candidate" ]] || fail "No JAR under app/core/build/libs - run './gradlew :pdf-elite:bootJar' first"
    fi
...
test_fixture() {
    ...
    mkdir -p "$configsdir"
    cp "$fixture_path" "$configsdir/pdf-elite-DB-2.3.232.mv.db"
    ...
    if (( rc == 0 )); then
        ...
        local preserved
        preserved="${MIGRATION_TEST_LOG_DIR:-/tmp}/PDFElite-migration-failed-$label"
        rm -rf "$preserved"
