...
Config (resolved via :class:`pdf_elite.config.AppSettings`/pydantic-settings):
...
class EngineSharedSecretMiddleware(BaseHTTPMiddleware):
    ...
    Secret/require values come from :class:`pdf_elite.config.AppSettings` by default; tests can
