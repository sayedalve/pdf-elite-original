...
public class GeneralUtils {
    ...
    public File convertMultipartFileToFile(MultipartFile multipartFile) throws IOException {
        String customTempDir = System.getenv("pdfelite_TEMPFILES_DIRECTORY");
        if (customTempDir == null || customTempDir.isEmpty()) {
            customTempDir = System.getProperty("pdfelite.tempfiles.directory");
        }
        ...
        if (customTempDir != null && !customTempDir.isEmpty()) {
            ...
            tempFile = Files.createTempFile(tempDir, "pdfelite-pdf-", null).toFile();
        } else {
            Path tempDir = Path.of(System.getProperty("java.io.tmpdir"), "pdfelite-pdf");
            if (!Files.exists(tempDir)) {
                Files.createDirectories(tempDir);
            }
            tempFile = Files.createTempFile(tempDir, "pdfelite-pdf-", null).toFile();
