...
export const ASSISTANT_ROUTES: AssistantRoute[] = [
  ...
  },
  {
    ...
    reply:
      "MCP wiring lives under Sources → Add → Agent. You'll get an MCP URL of the shape mcp://PDFElite.com/agents/{id} plus scoped credentials. Scenarios drive the eval set the agent must pass before it can take real traffic.",
  },
  {
  ...
  },
  {
    ...
    reply:
      "Outbound webhooks deliver via 3× exponential backoff. We sign every payload with HMAC-SHA256 — verify via the X-PDFElite-Signature header. Inbound webhooks support Bearer, HMAC, or mTLS auth.",
  },
];
