...
const mocks = vi.hoisted(() => ({
  ...
  configLoading: false,
  updatePDFEliteFileStub: vi.fn(),
  bumpRevision: vi.fn(),
  getPDFEliteFile: vi.fn(),
...
vi.mock("@app/contexts/FileContext", () => ({
  ...
  useFileManagement: () => ({
    updatePDFEliteFileStub: mocks.updatePDFEliteFileStub,
  }),
}));
...
vi.mock("@app/services/fileStorage", () => ({
  fileStorage: {
    getPDFEliteFile: (id: string) => mocks.getPDFEliteFile(id),
...
describe("useClientSideClassification delivery", () => {
  beforeEach(() => {
    ...
    mocks.updatePDFEliteFileStub.mockClear();
    mocks.bumpRevision.mockClear();
    mocks.updateFileMetadata.mockClear();
    mocks.meter.mockClear();
    mocks.getPDFEliteFile.mockReset();
    mocks.getPDFEliteFile.mockImplementation(async (id: string) =>
      fakeFile(id),
    );
  ...
  it("classifies pending uploads, writes labels, and meters once per file", async () => {
    ...
    await waitFor(() =>
      expect(mocks.updatePDFEliteFileStub).toHaveBeenCalledTimes(2),
    );
    expect(mocks.updatePDFEliteFileStub).toHaveBeenCalledWith("a", {
      classificationLabels: ["invoice"],
    });
    expect(mocks.updatePDFEliteFileStub).toHaveBeenCalledWith("b", {
  ...
  it("persists a definitive [] verdict for an unlabelled file and does not retry it", async () => {
    ...
    await waitFor(() =>
      expect(mocks.updatePDFEliteFileStub).toHaveBeenCalledWith("plain", {
  ...
  it("heals a previously-dispatched file whose result was lost, without re-metering", async () => {
    ...
    await waitFor(() =>
      expect(mocks.updatePDFEliteFileStub).toHaveBeenCalledWith("lost", {
  ...
  it("leaves an unreadable file undelivered (no verdict, no meter) so it can retry", async () => {
    ...
    expect(mocks.updatePDFEliteFileStub).not.toHaveBeenCalled();
  ...
  it("waits for app-config before classifying (AI flag unknown = possible double-run)", async () => {
    ...
    rerender();
    await waitFor(() =>
      expect(mocks.updatePDFEliteFileStub).toHaveBeenCalledWith("early", {
        classificationLabels: ["invoice"],
      }),
    );
  });
