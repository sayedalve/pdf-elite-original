...
import type { PDFEliteFile, PDFEliteFileStub } from "@app/types/fileContext";
...
function buildMeta(stub: PDFEliteFileStub): string {
  const parts: string[] = [];
...
export interface FileSelectorPickerProps {
  ...
  testId?: string;
  /**
   * Called with the stub (for display) and the ready-to-use PDFEliteFile (for processing).
...
export function FileSelectorPicker({
  ...
  const [savedStubs, setSavedStubs] = useState<PDFEliteFileStub[]>([]);
  ...
  const loadAndSelect = useCallback(
    async (stub: PDFEliteFileStub) => {
      if (loadingId) return;

      // Workbench file — get PDFEliteFile directly from FileContext (no loading needed)
      if (workbenchIdSet.has(stub.id)) {
        ...
        if (sf) {
          ...
          const workbenchStub =
            selectors.getPDFEliteFileStub(stub.id as FileId) ?? stub;
          onSelect({ stub: workbenchStub, PDFEliteFile: sf });
          setIsOpen(false);
        }
        return;
      }
      ...
      try {
        let PDFEliteFile: PDFEliteFile | null = null;
        ...
        if (stub.remoteShareToken) {
          ...
          if (files[0])
            PDFEliteFile = createPDFEliteFile(files[0], createFileId());
          ...
          if (files[0])
            PDFEliteFile = createPDFEliteFile(files[0], stub.id as FileId);
        } else {
          // Local IndexedDB file
          const localFile = await fileStorage.getPDFEliteFile(stub.id);
          if (localFile) PDFEliteFile = localFile;
        }
        ...
        if (PDFEliteFile) {
          ...
          onSelect({ stub: resolvedStub, PDFEliteFile });
          setIsOpen(false);
        }
      } catch (err) {
  ...
  const handleUpload = useCallback(
    async (file: File | null) => {
      ...
      try {
        ...
        let stub = createNewPDFEliteFileStub(file, id);
        const PDFEliteFile = createPDFEliteFile(file, id);
        ...
        await fileStorage.storePDFEliteFile(PDFEliteFile, stub);
        lsSet(LS_TAB, "saved");
        setActiveTab("saved");
        const refreshed = await loadRecentFiles();
        setSavedStubs(refreshed);
        onSelect({ stub, PDFEliteFile });
        setIsOpen(false);
      } catch (err) {
