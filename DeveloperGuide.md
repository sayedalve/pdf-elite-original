# PDF Elite Developer Guide

## 1. Introduction

PDF Elite is a robust, locally hosted, web-based PDF manipulation tool. **Stirling 2.0** represents a complete frontend rewrite with a modern React SPA (Single Page Application).
...
1. Clone the repository:
   ...
   ```bash
   git clone https://github.com/Stirling-Tools/PDF Elite.git
   cd PDF Elite
   ```

2. Install Docker and JDK 21 if not already installed.
...
4. Lombok Setup
PDF Elite uses Lombok to reduce boilerplate code. Some IDEs, like Eclipse, don't support Lombok out of the box. To set up Lombok in your development environment:
Visit the [Lombok website](https://projectlombok.org/setup/) for installation instructions specific to your IDE.

5. Add environment variable
For local testing, you should generally be testing the full 'Security' version of PDF Elite. To do this, you must add the environment flag DISABLE_ADDITIONAL_FEATURES=false to your system and/or IDE build/run step.
...
PDF Elite can be packaged as a cross-platform desktop application using Tauri with PDF file association support and bundled JRE.
...
PDF Elite offers several Docker versions:
...
PDF Elite provides several example Docker Compose files in the `exampleYmlFiles` directory, such as:
...
services:
  PDF Elite:
    container_name: PDF Elite-Security
    image: docker.stirlingpdf.com/stirlingtools/PDF Elite:latest
    deploy:
      resources:
        limits:
          memory: 4G
    ...
    environment:
      ...
      UI_APPNAME: PDF Elite
      UI_HOMEDESCRIPTION: Demo site for PDF Elite Latest with Security
      UI_APPNAMENAVBAR: PDF Elite Latest
...
PDF Elite uses different Docker images for various configurations. The build process is controlled by environment variables and uses specific Dockerfile variants. Here's how to build the Docker images:
...
3. Build the Docker images:
   ...
   ```bash
   docker build --no-cache --pull --build-arg VERSION_TAG=alpha -t stirlingtools/PDF Elite:latest -f ./Dockerfile .
   ```

   For the ultra-lite version:

   ```bash
   docker build --no-cache --pull --build-arg VERSION_TAG=alpha -t stirlingtools/PDF Elite:latest-ultra-lite -f ./Dockerfile.ultra-lite .
   ```
   ...
   docker build --no-cache --pull --build-arg VERSION_TAG=alpha -t stirlingtools/PDF Elite:latest-fat -f ./Dockerfile.fat .
   ```
...
PDF Elite also provides a `test.sh` script in the root directory for Docker integration tests. This script builds all versions of PDF Elite, checks that each version works, and runs Cucumber tests. It's recommended to run this script before submitting a final pull request.
...
For quick iterations and development of Java backend, JavaScript, and UI components, you can run and test PDF Elite locally without Docker. This approach allows you to work on and verify changes to:
...
8. See additional [contributing guidelines](https://github.com/Stirling-Tools/PDF Elite/blob/main/CONTRIBUTING.md).
...
API documentation is available at `/swagger-ui/index.html` when running the application. You can also view the latest API documentation [here](https://app.swaggerhub.com/apis-docs/Stirling-Tools/PDF Elite/).

## 10. Customization

PDF Elite can be customized through environment variables or a `settings.yml` file. Key customization options include:
...
```bash
docker run -p 8080:8080 -e APP_NAME="My PDF Tool" PDF Elite:full
```
...
For managing language translations that affect multiple files, PDF Elite provides a helper script:
...
1. **Create a New Controller:**
   - Create a new Java class in the `PDF Elite/src/main/java/stirling/software/SPDF/controller/api` directory.
...
2. **Define the Service Layer:** (Not required but often useful)
   - Create a new service class in the `PDF Elite/src/main/java/stirling/software/SPDF/service` directory.
...
## Adding New Translations to Existing Language Files in PDF Elite

When adding a new feature or modifying existing ones in PDF Elite, you'll need to add new translation entries to the existing language files. Here's a step-by-step guide:

### 1. Locate Existing Language Files

Find the existing `messages.properties` files in the `PDF Elite/src/main/resources` directory. You'll see files like:
