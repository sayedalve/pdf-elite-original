...
import { usePdfTextEditorWorkbenchBarButtons } from "@app/components/tools/pdfTextEditor/usePdfTextEditorWorkbenchBarButtons";
...
import { convertPdfToJsonLocal } from "@app/tools/pdfTextEditor/pdfToJsonLocal";
...
const PdfTextEditor = ({ onComplete, onError }: BaseToolProps) => {
  ...
  const handleLoadFile = useCallback(
    async (file: File | null) => {
      ...
      try {
        ...
        if (isPdf) {
          // ── LOCAL (offline) conversion — replaces removed backend endpoint ──
  ...
  const handleSaveToWorkbench = useCallback(async () => {
    ...
    try {
      ...
      const parentStub = selectors.getPDFEliteFileStub(sourceFileId);
      if (!parentStub) {
        console.warn(
      ...
      // Create PDFEliteFile and stub for the output
      ...
      await consumeFiles([sourceFileId], PDFEliteFiles, stubs);
  ...
  // Hook up the WorkbenchBar buttons
  usePdfTextEditorWorkbenchBarButtons(viewData);
