...
import { useGlobalPaste } from "@app/hooks/useGlobalPaste";
...
import { createPDFEliteFilesAndStubs } from "@app/services/fileStubHelpers";
import { isPDFEliteFile, getFormFillFileId } from "@app/types/fileContext";
...
import { DocumentTabBar } from "@app/components/viewer/DocumentTabBar";
import { useFormFill } from "@app/tools/formFill/FormFillContext";
import { FormSaveBar } from "@app/tools/formFill/FormSaveBar";
import { ContextualLeftPanel } from "@app/components/viewer/ContextualLeftPanel";
...
const EmbedPdfViewerContent = ({
  ...
  useGlobalPaste(annotationApiRef);
  ...
  const currentFile = React.useMemo(() => {
    if (previewFile) {
      ...
      const byId = activeFileId
        ? activeFiles.find(
            (f) => isPDFEliteFile(f) && f.fileId === activeFileId,
          )
        : null;
  ...
  const currentFileStableId =
    currentFile && isPDFEliteFile(currentFile) ? currentFile.fileId : null;
  ...
  const isCurrentFileEncrypted = React.useMemo(() => {
    if (!currentFile || !isPDFEliteFile(currentFile)) return false;
    const stub = selectors.getPDFEliteFileStub(currentFile.fileId);
    return stub?.processedFile?.isEncrypted === true;
  }, [currentFile, selectors]);

  const bookmarkCacheKey = React.useMemo(() => {
    if (currentFile && isPDFEliteFile(currentFile)) {
  ...
  // ─── Page Memory: save page on scroll ────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    if (hasRestoredPageRef.current === bookmarkCacheKey) return;

    const restoredPage = pageMemoryService.getPage(bookmarkCacheKey);
    if (restoredPage <= 1) {
      hasRestoredPageRef.current = bookmarkCacheKey;
      return;
    }
    ...
    if (scrollState.totalPages > 0) {
      hasRestoredPageRef.current = bookmarkCacheKey;
      const targetPage = Math.min(restoredPage, scrollState.totalPages);
      if (targetPage > 1) {
  ...
  useWheelZoom({
    ref: viewerRef,
    zoomContainerRef: pdfContainerRef,
    onZoomIn: zoomActions.zoomIn,
    onZoomOut: zoomActions.zoomOut,
    // Wire smooth continuous zoom so trackpad pinch gestures feel native.
    // getZoomFactor must return the CURRENT zoom as a scale factor in the same
    // unit setZoomLevel consumes (where PDF.js 1.0 scale = 75% visual).
    // So 100% zoom = 1.333 factor.
  ...
  const applyChanges = useCallback(async () => {
    ...
    const saveChanges = async () => {
      ...
      // Step 3: Create PDFEliteFiles and stubs for version history
      ...
      const parentStub = selectors.getPDFEliteFileStub(currentFileId);
      if (!parentStub) throw new Error("Parent stub not found");

      const { PDFEliteFiles, stubs } = await createPDFEliteFilesAndStubs(
        [file],
        parentStub,
      ...
      await actions.consumeFiles([currentFileId], PDFEliteFiles, stubs);
  ...
  const handleFormApply = useCallback(
    async (filledBlob: Blob) => {
      ...
      try {
        ...
        // Create PDFEliteFiles and stubs for version history
        const { PDFEliteFiles, stubs } = await createPDFEliteFilesAndStubs(
          [file],
          parentStub,
          selectedTool ?? "multiTool",
  ...
  return (
    ...
    >
      {/* Document Tab Bar - shows open documents as browser-like tabs.
          Hidden in preview mode (tool output preview) which uses its own close button. */}
      {!previewFile && (
        <Box
          style={{
            marginLeft: `${totalLeftMargin}rem`,
          ...
          >
            <LocalEmbedPDF
              ...
              fileName={
                previewFile
                  ? previewFile.name
                  : currentFile && isPDFEliteFile(currentFile)
                    ? currentFile.name
      ...
      <ContextualLeftPanel
        ...
        bookmarkCacheKey={bookmarkCacheKey ?? ""}
        allBookmarkCacheKeys={allBookmarkCacheKeys}
        effectiveFile={effectiveFile}
        handleLayerApply={handleLayerApply}
      />
    </Box>
  );
};
