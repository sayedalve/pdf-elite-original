...
import { isPDFEliteFile, type FileId } from "@app/types/fileContext";
...
export const BookmarkSidebar = ({
  ...
  const handleSubmitAddBookmark = useCallback(async () => {
    ...
    const resolvedFile = activeFileId
      ? allFiles.find((f) => isPDFEliteFile(f) && f.fileId === activeFileId)
      : (allFiles[activeFileIndex] ?? allFiles[0]);
    const resolvedFileId =
      resolvedFile && isPDFEliteFile(resolvedFile)
    ...
    const parentStub = selectors.getPDFEliteFileStub(fileId);
    if (!file || !parentStub) {
      handleFallbackToTool();
      return;
    }
