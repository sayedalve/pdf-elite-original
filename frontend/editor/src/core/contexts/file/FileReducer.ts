...
export function fileContextReducer(
  ...
  switch (action.type) {
    case "ADD_FILES": {
      const { PDFEliteFileStubs } = action.payload;
      const newIds: FileId[] = [];
      const newById: Record<FileId, PDFEliteFileStub> = { ...state.files.byId };

      PDFEliteFileStubs.forEach((record) => {
    ...
    case "CONSUME_FILES": {
      const { inputFileIds, outputPDFEliteFileStubs, silent } = action.payload;
      ...
      const provenancedOutputs = outputPDFEliteFileStubs.map((stub) => ({
        ...stub,
    ...
    case "UNDO_CONSUME_FILES": {
      const { inputPDFEliteFileStubs, outputFileIds } = action.payload;

      return processFileSwap(state, outputFileIds, inputPDFEliteFileStubs);
    }
