...
export class AuthService {
  ...
  private async saveTokenEverywhere(
    ...
    if (refreshToken) {
      try {
        ...
        localStorage.removeItem("PDFElite_refresh_token");
      } catch (error) {
        console.error(
  ...
  private async getTokenFromAnySource(): Promise<string | null> {
    ...
    return localStorage.getItem("PDFElite_jwt");
  }
  ...
  async signUpSaas(email: string, password: string): Promise<void> {
    ...
    const signupUrl = `${PDFElite_SAAS_URL.replace(/\/$/, "")}/auth/v1/signup?redirect_to=${redirectParam}`;
  ...
  async login(
    ...
    try {
      ...
      if (serverUrl === PDFElite_SAAS_URL) {
        if (!PDFElite_SAAS_URL) {
          throw new Error("VITE_SAAS_SERVER_URL is not configured");
        }
        if (!SUPABASE_KEY) {
      ...
      const response = await invoke<LoginResponse>("login", {
        ...
        saasServerUrl: PDFElite_SAAS_URL,
      });
      ...
      if (error instanceof Error || typeof error === "string") {
        ...
        // 404 - endpoint not found
          ...
          this.setAuthStatus("unauthenticated", null);
          throw new Error(
            "Login endpoint not found. Please ensure you are connecting to a valid PDF Elite server.",
            {
              cause: error,
            },
          );
    ...
    if (path.startsWith("/login") || path.startsWith("/setup")) {
      ...
      const tokenInStorage =
        typeof window !== "undefined"
          ? localStorage.getItem("PDFElite_jwt")
          : null;
      if (tokenInStorage) {
        console.log(
