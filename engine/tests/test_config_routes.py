...
from pdf_elite.api import app
from pdf_elite.api.app import _adopt_cached_config_if_changed
from pdf_elite.config import AppSettings, config_cache, load_settings
from pdf_elite.contracts import ConfigPushRequest
from pdf_elite.documents import DocumentService
...
def test_config_push_persist_failure_still_applies_without_500() -> None:
    ...
    with _client(build_app_settings) as client:
        with patch(
            "pdf_elite.api.routes.config.save_config",
...
def test_config_push_unsupported_model_returns_400() -> None:
    ...
    with _client(build_app_settings) as client:
        before = app.state.runtime
        with patch(
            "pdf_elite.api.routes.config.validate_structured_output_support",
