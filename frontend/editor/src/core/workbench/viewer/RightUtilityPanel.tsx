...
import { useViewerScroll, useViewerZoom } from "@app/contexts/ViewerContext";
...
export const RightUtilityPanel: React.FC<RightUtilityPanelProps> = ({
  ...
  const viewerScroll = useViewerScroll();
  const viewerZoom = useViewerZoom();

  const page = pageProp ?? viewerScroll.page.current;
  const totalPages = totalPagesProp ?? viewerScroll.page.total;
  const onPageChange =
    onPageChangeProp ?? viewerScroll.scrollActions.scrollToPage;

  const zoom = zoomProp ?? {
    percentage: viewerZoom.zoom.percentage,
    scale: viewerZoom.zoom.scale,
  };
  ...
  const tabs: {
    ...
    { id: "pages", label: "Pages", icon: <Hash size={18} /> },
    {
      id: "bookmarks",
      label: "Outline",
  ...
  if (collapsed) {
    return (
      <div
        className={`rup-collapsed-rail ${className || ""}`}
        role="complementary"
        aria-label="Collapsed utility panel"
      >
        <button
        ...
        {!hidePageNavigation && onPageChange && (
        ...
        <style>{`
          ...
          .rail-page-nav {
            ...
            gap: 8px;
            margin-top: auto;
            margin-bottom: auto; /* Vertically center in remaining space */
            background: var(--surface-elevated, #2a2d3f);
            border: 1px solid var(--border-strong, #4a5075);
            border-radius: 12px;
            padding: 10px 6px;
            width: 48px;
          }
          ...
          .page-input {
            width: 40px;
            background: var(--surface-elevated-bright, #343953);
            border: 1px solid var(--border-strong, #4a5075);
            border-radius: 6px;
            color: #ffffff;
            font-weight: 700;
          ...
          .page-input:focus {
            ...
            border-color: var(--accent, #79AEFF);
          }
          .page-sep, .page-total {
            color: var(--text-secondary, #9aa0b8);
            font-size: 13px;
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        .rup-compact-bm-title { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
