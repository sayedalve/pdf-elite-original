...
import pdfelite.software.SPDF.model.api.security.HardwareCertificateInfo;
import pdfelite.software.SPDF.model.api.security.HardwareSigningCapabilities;
import pdfelite.software.SPDF.model.api.security.HardwareSigningCapabilities.Pkcs11LibraryInfo;
...
public class HardwareKeyStoreService {
    ...
    private static final String PKCS11_LIBRARIES_ENV = "pdfelite_PKCS11_LIBRARIES";

    /** Same as {@link #PKCS11_LIBRARIES_ENV} but as a JVM system property. */
    private static final String PKCS11_LIBRARIES_PROP = "pdfelite.pkcs11.libraries";
     ...
     * bundle sets {@code pdfelite_PDF_TAURI_MODE=true} (with {@code BROWSER_OPEN=false}, so
    ...
    public boolean isDesktop() {
        if (Boolean.parseBoolean(System.getProperty("pdfelite_PDF_TAURI_MODE", "false"))) {
            return true;
        }
    ...
    public void assertLocalDesktop(HttpServletRequest request) {
        if (!isDesktop()) {
            throw ExceptionUtils.createIllegalArgumentException(
                    ...
                    "Hardware-backed signing is only available in the pdfelite PDF desktop app");
        }
        ...
        if (!ok) {
            throw ExceptionUtils.createIllegalArgumentException(
                    ...
                    "PKCS#11 driver is not in the allowed list. Add it via the"
                            + " pdfelite_PKCS11_LIBRARIES setting: {0}",
                    libraryPath);
        }
    }
