...
export async function uploadHistoryChain(
...
): Promise<{ remoteId: number; updatedAt: number; chain: PDFEliteFileStub[] }> {
  ...
  const finalFile = await fileStorage.getPDFEliteFile(finalStub.id);
  if (!finalFile) {
...
export async function uploadHistoryChains(
  ...
  const chainMap = new Map<FileId, PDFEliteFileStub[]>();
  const combinedChain: PDFEliteFileStub[] = [];
  ...
  if (leafStubs.length === 1) {
    const finalFile = await fileStorage.getPDFEliteFile(leafStubs[0].id);
    if (!finalFile) {
      throw new Error("Missing final file data for sharing.");
    }
    shareFile = finalFile;
