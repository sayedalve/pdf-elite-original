...
const DEBUG_FLAG = "PDFElite-classification-debug";
...
export function useClientSideClassification(): void {
  ...
  const { updatePDFEliteFileStub } = useFileManagement();
  ...
  useEffect(() => {
    ...
    const claimKey = (s: PDFEliteFileStub) =>
    ...
    const cancelIdle = scheduleIdle(() => {
      ...
      void (async () => {
        ...
        for (const stub of pending) {
          ...
          updatePDFEliteFileStub(stub.id as FileId, {
...
async function classifyStub(
  ...
  let file: PDFEliteFile | null = null;
  for (let i = 0; i < FILE_WAIT_TRIES; i++) {
    file = await fileStorage.getPDFEliteFile(fileId).catch(() => null);
    if (file) break;
