...
function prerenderOgPlugin(isSaas: boolean): PluginOption {
  // SaaS (PDFElite.com) prerenders the marketing cards from a dedicated
...
export default defineConfig(async ({ mode, command }) => {
  ...
  const devWorktreeLabel =
    command === "serve" ? (process.env.PDFElite_DEV_LABEL ?? "") : "";
  ...
  // Effective mode: --mode > PDFElite_FLAVOR > ENABLE_SAAS > DISABLE_ADDITIONAL_FEATURES > proprietary.
  const explicitMode = (VALID_MODES as readonly string[]).includes(mode)
    ? (mode as BuildMode)
    : null;
  const flavor = (process.env.PDFElite_FLAVOR ?? "").toLowerCase();
