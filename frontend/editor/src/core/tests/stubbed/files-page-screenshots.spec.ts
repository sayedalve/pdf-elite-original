...
async function seedFiles(page: Page, files: SeedFile[]): Promise<void> {
  ...
  await page.addInitScript(
    ({ records, dbVersion }) => {
      const open = window.indexedDB.open("pdf-elite-files", dbVersion);
...
test.describe("Files page screenshots", () => {
  ...
  async function enableRtl(page: Page): Promise<void> {
    ...
    await page.addInitScript(() => {
      ...
      localStorage.setItem("PDFElite-language", "ar-AR");
      localStorage.setItem("PDFElite-language-source", "user");
