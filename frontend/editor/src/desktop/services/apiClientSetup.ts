  ...
  client.interceptors.request.use(
    async (config: InternalAxiosRequestConfig) => {
      ...
      try {
        ...
        const saasBase = PDFElite_SAAS_BACKEND_API_URL ?? "";
        const finalUrl = extendedConfig.url ?? "";
        const isSaaSBackendRequest =
          baseUrl === PDFElite_SAAS_BACKEND_API_URL ||
    ...
    async (error) => {
      ...
      if (error.response?.status === 401 && !originalRequest._retry) {
        ...
        if (isRemote) {
          ...
          refreshed = await authService.refreshSupabaseToken(PDFElite_SAAS_URL);
        }
