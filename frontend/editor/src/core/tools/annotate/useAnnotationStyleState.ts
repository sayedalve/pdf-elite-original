...
function usePersistedColor(key: string, defaultColor: string) {
...
export const useAnnotationStyleState = (
  ...
  const [inkColor, setInkColor] = usePersistedColor(
    "pdf-elite-ink-color",
    "#1f2933",
  );
  const [inkWidth, setInkWidth] = useState(2);
  const [highlightColor, setHighlightColorState] = useState(() => {
    return localStorage.getItem("pdf-elite-highlight-color") || "#ffd54f";
  });
  ...
  useEffect(() => {
    const handleStorageChange = (e: CustomEvent | Event) => {
      const customEvent = e as CustomEvent;
      if (
        customEvent.detail?.type === "highlight" &&
        customEvent.detail?.color
      ) {
        setHighlightColorState(customEvent.detail.color);
      }
    };
    window.addEventListener("pdf-elite-color-change", handleStorageChange);
    return () =>
      window.removeEventListener("pdf-elite-color-change", handleStorageChange);
  }, []);
  ...
  const [underlineColor, setUnderlineColor] = usePersistedColor(
    "pdf-elite-underline-color",
    "#ffb300",
  );
  const [underlineOpacity, setUnderlineOpacity] = useState(100);
  const [strikeoutColor, setStrikeoutColor] = usePersistedColor(
    "pdf-elite-strikeout-color",
    "#e53935",
  );
  const [strikeoutOpacity, setStrikeoutOpacity] = useState(100);
  const [squigglyColor, setSquigglyColor] = usePersistedColor(
    "pdf-elite-squiggly-color",
    "#00acc1",
  );
  const [squigglyOpacity, setSquigglyOpacity] = useState(100);
  const [textColor, setTextColor] = usePersistedColor(
    "pdf-elite-text-color",
    "#111111",
  );
  ...
  const [noteBackgroundColor, setNoteBackgroundColor] = usePersistedColor(
    "pdf-elite-note-color",
    "#ffd54f",
  );

  const [shapeStrokeColor, setShapeStrokeColor] = usePersistedColor(
    "pdf-elite-shape-stroke",
    "#cf5b5b",
  );
  const [shapeFillColor, setShapeFillColor] = usePersistedColor(
    "pdf-elite-shape-fill",
    "#0000ff",
  );
