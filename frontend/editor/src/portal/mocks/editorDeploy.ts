 ...
 *
 * This surface manages the org's deployment of the PDF Elite *Editor* product
...
const DOCKER_SNIPPET = `docker run -d --name PDFElite-editor \\
  -p 8080:8080 \\
  -e PDFElite_ORG_PAIRING_TOKEN="$PAIRING_TOKEN" \\
  -e PDFElite_REGION="us-east-1" \\
  -v PDFElite-data:/var/lib/PDFElite \\
  PDFElitepdf/editor:3.2.1`;

const HELM_SNIPPET = `helm repo add PDFElite https://charts.PDFElitepdf.com
helm install editor PDFElite/editor \\
  --namespace PDFElite --create-namespace \\
...
https://app.PDFElitepdf.com/o/acme/editor`;
...
function targetsFor(tier: Tier): DeploymentTarget[] {
  const cloud: DeploymentTarget = {
    kind: "cloud",
    label: "Managed Cloud",
    tagline: "PDFElite-hosted, zero-ops. Available on every plan.",
    state: "running",
    requiresTier: "free",
  ...
  return [
    {
      ...
      label: "Pairing token",
      description:
        "Long-lived secret injected as PDFElite_ORG_PAIRING_TOKEN. Rotate if it leaks.",
      requiresTier: "free",
    ...
    },
    {
      ...
      value: iacUnlocked
        ? 'module "PDFElite_editor" { org_id = "acme" }'
        : "Enterprise only",
      locked: !iacUnlocked,
    },
  ];
...
function summaryFor(
  ...
  return {
    ...
    workspaceUrl: "https://app.PDFElitepdf.com/o/acme/editor",
  };
}
