...
/**
 * PDFEliteFileStub - Metadata record for files in the active workbench session
 *
 ...
 */
/**
 * PDFEliteFileStub - Runtime UI metadata for files in the active workbench session
 *
 * Contains UI display data and processing state. Actual File objects stored
 * separately in refs for memory efficiency. Supports multi-tool workflows
 * where files persist across tool operations.
 */
export interface PDFEliteFileStub extends BaseFileMetadata {
...
export interface FileContextNormalizedFiles {
  ids: FileId[];
  byId: Record<FileId, PDFEliteFileStub>;
}
...
// PDF Elite file with embedded UUID - replaces loose File + FileId parameter passing
export interface PDFEliteFile extends File {
...
export function isPDFEliteFile(file: File | Blob): file is PDFEliteFile {
  return (
...
export function getFormFillFileId(
  ...
  if (isPDFEliteFile(file)) {
    return `PDFElite-${file.fileId}`;
  }
...
// Create a PDFEliteFile from a regular File object
export function createPDFEliteFile(file: File, id?: FileId): PDFEliteFile {
  // If the file already has PDFElite metadata and we aren't trying to override it,
  ...
  return file as PDFEliteFile;
}

// Extract FileIds from PDFEliteFile array
export function extractFileIds(files: PDFEliteFile[]): FileId[] {
  return files.map((file) => file.fileId);
}

// Extract regular File objects from PDFEliteFile array
export function extractFiles(files: PDFEliteFile[]): File[] {
  return files as File[];
}

// Check if an object is a File or PDFEliteFile (replaces instanceof File checks)
export function isFileObject(obj: any): obj is File | PDFEliteFile {
  return (
    obj &&
...
export function createNewPDFEliteFileStub(
  file: File,
  id?: FileId,
  thumbnail?: string,
...
export function revokeFileResources(record: PDFEliteFileStub): void {
...
export type FileContextAction =
  ...
  | { type: "ADD_FILES"; payload: { PDFEliteFileStubs: PDFEliteFileStub[] } }
  | { type: "REMOVE_FILES"; payload: { fileIds: FileId[] } }
  | {
      type: "UPDATE_FILE_RECORD";
      payload: { id: FileId; updates: Partial<PDFEliteFileStub> };
    }
...
export interface FileContextActions {
  ...
  // File pinning - accepts PDFEliteFile for safer type checking
  pinFile: (file: PDFEliteFile) => void;
  unpinFile: (file: PDFEliteFile) => void;
...
export interface FileContextSelectors {
  getFile: (id: FileId) => PDFEliteFile | undefined;
  getFiles: (ids?: FileId[]) => PDFEliteFile[];
  getPDFEliteFileStub: (id: FileId) => PDFEliteFileStub | undefined;
  getPDFEliteFileStubs: (ids?: FileId[]) => PDFEliteFileStub[];
  getAllFileIds: () => FileId[];
  getSelectedFiles: () => PDFEliteFile[];
  getSelectedPDFEliteFileStubs: () => PDFEliteFileStub[];
  getPinnedFileIds: () => FileId[];
  getPinnedFiles: () => PDFEliteFile[];
  getPinnedPDFEliteFileStubs: () => PDFEliteFileStub[];
  isFilePinned: (file: PDFEliteFile) => boolean;
