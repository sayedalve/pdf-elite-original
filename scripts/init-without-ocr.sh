#!/bin/bash
# This script initializes PDF Elite without OCR features.
set -euo pipefail
...
if [ -x /scripts/PDFElite-diagnostics.sh ]; then
  mkdir -p /usr/local/bin
  ln -sf /scripts/PDFElite-diagnostics.sh /usr/local/bin/diagnostics
  ln -sf /scripts/PDFElite-diagnostics.sh /usr/local/bin/PDFElite-diagnostics
  ln -sf /scripts/PDFElite-diagnostics.sh /usr/local/bin/diag
  ln -sf /scripts/PDFElite-diagnostics.sh /usr/local/bin/debug
  ln -sf /scripts/PDFElite-diagnostics.sh /usr/local/bin/diagnostic
fi
if [ -x /scripts/aot-diagnostics.sh ] && [ "${PDFElite_AOT_ENABLE:-false}" = "true" ]; then
  mkdir -p /usr/local/bin
...
warn_switch_user_once() {
  if [ "$SWITCH_USER_WARNING_EMITTED" = false ]; then
    log "WARNING: Unable to switch to user ${RUNTIME_USER:-PDFElitepdfuser}; running command as ${CURRENT_USER}."
...
if [ -z "${VERSION_TAG:-}" ] && [ -f /etc/PDFElite_version ]; then
  VERSION_TAG="$(tr -d '\r\n' < /etc/PDFElite_version)"
  export VERSION_TAG
fi

# ---------- AOT ----------
# OFF by default. Set PDFElite_AOT_ENABLE=true to opt in.
AOT_ENABLED="${PDFElite_AOT_ENABLE:-false}"
...
generate_aot_cache() {
  ...
  local aot_conf="/tmp/PDFElite.aotconf"
  local arch
  arch=$(uname -m)
  ...
  if command_exists timeout; then
    ...
    timeout "${record_timeout}s" \
      java "-Xmx${record_xmx}" ${COMPACT_HEADERS_FLAG:-} ${COMPRESSED_OOPS_FLAG} \
           ...
           -DPDFElite.datasource.url="jdbc:h2:mem:aottraining;DB_CLOSE_DELAY=-1;MODE=PostgreSQL" \
  ...
  if [ "$record_exit" -eq 137 ]; then
    ...
    log "AOT: Set PDFElite_AOT_ENABLE=false or increase container memory above 1GB"
    rm -f "$aot_conf" /tmp/aot-record.log
    return 1
  fi
...
JVM_PROFILE="${PDFElite_JVM_PROFILE:-balanced}"
...
# Priority: JAVA_BASE_OPTS (explicit override) > PDFElite_JVM_PROFILE > fallback defaults
  ...
  case "$JVM_PROFILE" in
    ...
    *)
      if [ -n "${_JVM_OPTS_BALANCED:-}" ]; then
        ...
        JAVA_BASE_OPTS="-XX:+ExitOnOutOfMemoryError -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/tmp/pdf-elite/heap_dumps -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:G1HeapRegionSize=4m -XX:G1PeriodicGCInterval=60000 -XX:+UseStringDeduplication -XX:+UseCompactObjectHeaders -XX:+ExplicitGCInvokesConcurrent -Dspring.threads.virtual.enabled=true"
      fi
      ;;
  esac
...
AOT_CACHE="/configs/cache/PDFElite.aot"
...
if [ "$AOT_ENABLED" = "true" ]; then
  ...
  if [ "$AOT_SUPPORTED" = false ]; then
    ...
    rm -f /app/PDFElite.jsa /app/PDFElite.aot /app/PDFElite.aot.fingerprint 2>/dev/null || true
  else
...
log "Running PDF Elite with DISABLE_ADDITIONAL_FEATURES=${DISABLE_ADDITIONAL_FEATURES:-} and VERSION_TAG=${VERSION_TAG:-<unset>}"
...
if [[ "${INSTALL_BOOK_AND_ADVANCED_HTML_OPS:-false}" == "true" && "${FAT_DOCKER:-true}" != "true" ]]; then
  log "issue with calibre in current version, feature currently disabled on pdf-elite"
fi
...
if [ "$(id -u)" -eq 0 ]; then
  if id -u PDFElitepdfuser >/dev/null 2>&1; then
    if [ -n "${PUID:-}" ] && [ "$PUID" != "$(id -u PDFElitepdfuser)" ]; then
      usermod -o -u "$PUID" PDFElitepdfuser || true
      chown PDFElitepdfuser:PDFElitepdfgroup "${XDG_RUNTIME_DIR}" 2>/dev/null || true
    fi
  fi
  if getent group PDFElitepdfgroup >/dev/null 2>&1; then
    if [ -n "${PGID:-}" ] && [ "$PGID" != "$(getent group PDFElitepdfgroup | cut -d: -f3)" ]; then
      groupmod -o -g "$PGID" PDFElitepdfgroup || true
    fi
  fi
fi
...
mkdir -p /tmp/pdf-elite /tmp/pdf-elite/heap_dumps /logs /configs /configs/heap_dumps /configs/cache /customFiles /pipeline /storage || true
CHOWN_PATHS=("$HOME" "/logs" "/scripts" "/configs" "/customFiles" "/pipeline" "/storage" "/tmp/pdf-elite" "/app.jar")
[ -d /usr/share/fonts/truetype ] && CHOWN_PATHS+=("/usr/share/fonts/truetype")
CHOWN_OK=true
for p in "${CHOWN_PATHS[@]}"; do
  if [ -e "$p" ]; then
    chown -R "PDFElitepdfuser:PDFElitepdfgroup" "$p" 2>/dev/null || CHOWN_OK=false
...
# ---------- Java ----------
# Start PDF Elite Java application immediately (parallel with unoserver startup).
log "Starting PDF Elite"
JAVA_CMD=(
  java
  -Dfile.encoding=UTF-8
  -Djava.io.tmpdir=/tmp/pdf-elite
)
...
if [ -f "/app.jar" ]; then
  ...
  JAVA_CMD+=("-cp" "/app/app.jar:/app/lib/*" "PDFElite.software.SPDF.SPDFApplication")
else
...
if [ "$AOT_GENERATE_BACKGROUND" = true ]; then
  ...
  if [ "$CONTAINER_MEM_MB" -gt "$_aot_min_mem" ] || [ "$CONTAINER_MEM_MB" -eq 0 ]; then
    (
      ...
      while [ "$_attempt" -le "$_max_attempts" ]; do
        ...
        if [ -f /app/app.jar ] && [ -d /app/lib ]; then
          generate_aot_cache "$AOT_CACHE" \
            -cp "/app/app.jar:/app/lib/*" PDFElite.software.SPDF.SPDFApplication || _gen_rc=$?
        elif [ -f /app.jar ]; then
      ...
      log "AOT: To disable, set PDFElite_AOT_ENABLE=false (or omit it, default is off)"
    ) &
    AOT_GEN_PID=$!
...
exit_code=$?
case "$exit_code" in
  0)   log "PDF Elite exited normally." ;;
  137) log "PDF Elite was OOM-killed (exit 137). Check container memory limits." ;;
  143) log "PDF Elite terminated by SIGTERM (normal orchestrator shutdown)." ;;
  *)   log "PDF Elite exited with code ${exit_code}." ;;
esac
