...
import { StickyNoteEditor } from "@app/components/viewer/StickyNoteEditor";
...
function AnnotationSelectionMenuInner({
  ...
  // Keyboard shortcut to delete annotation
  useEffect(() => {
    if (!selected) return;
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Delete" || e.key === "Backspace") {
        // Do not delete the annotation if the user is typing inside an input, textarea, or contenteditable
        ...
        if (!isInput) {
          e.preventDefault(); // Prevent browser navigation (Backspace)
          handlers.onDelete();
        }
      }
    };
  ...
  const isStickyNote = annotation.object.type === PdfAnnotationSubtype.TEXT;
  ...
  const menuContent = menuPosition ? (
    <div
      ...
      style={{
        ...
        borderRadius: 8,
        padding: isStickyNote ? "16px" : "8px 12px",
        boxShadow: "0 4px 16px rgba(0, 0, 0, 0.3)",
        border: "1px solid var(--mantine-color-default-border)",
        fontSize: "14px",
        minWidth: isStickyNote ? "280px" : `${handlers.menuWidth}px`,
    ...
    >
      {isStickyNote ? (
        <StickyNoteEditor
          ...
          onUpdate={handlers.onPropertiesUpdate}
          onDelete={handlers.onDelete}
          onSave={() => provides?.deselectAnnotation?.()}
        />
      ) : (
