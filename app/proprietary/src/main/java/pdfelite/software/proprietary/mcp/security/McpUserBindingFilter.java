package pdfelite.software.proprietary.mcp.security;
...
/**
 * Binds an MCP-validated JWT to a provisioned pdfelite user: optionally rejects subjects with no
 * enabled account, then rebinds the principal to the canonical pdfelite username (scope authorities
        ...
        if (current instanceof JwtAuthenticationToken jwtAuth && jwtAuth.isAuthenticated()) {
            ...
            if (requireExistingAccount) {
                ...
                if (account.isEmpty() || !account.get().isEnabled()) {
                    log.warn(
                            "MCP access denied: token subject '{}' has no active pdfelite account",
                            sanitizeForLog(username));
                    reject(
                            response,
                            "MCP access requires a provisioned, enabled pdfelite account for this"
                                    + " subject.");
                    return;
                }
            ...
            // Rebind to the pdfelite username, carrying only the OAuth scope authorities.
