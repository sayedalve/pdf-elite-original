...
const TOOL_DEFAULT_BUILDERS: Record<AnnotationToolId, ToolDefaultsBuilder> = {
  ...
  text: (options) => ({
    ...
    textColor: options?.color ?? DEFAULTS.text,
    fontSize: options?.fontSize ?? 14,
    fontFamily: options?.fontFamily ?? "Helvetica",
    textAlign: options?.textAlign ?? 0,
    opacity: options?.opacity ?? 1,
    borderWidth: options?.thickness ?? 0, // No border by default for plain text
    contents: options?.contents ?? "Text", // Provide default text
    ...(options?.fillColor
      ? { color: options.fillColor, interiorColor: options.fillColor }
      : { color: undefined, interiorColor: undefined }), // Ensure it's transparent by default
    ...withCustomData(options),
  }),
  note: (options) => {
    return {
      ...
      icon: getIconEnum(options?.icon) ?? "Comment",
      ...withCustomData(options),
    };
  },
  square: (options) => ({
