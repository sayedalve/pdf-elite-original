...
export function useOpenWindowFiles() {
  ...
  useEffect(() => {
    ...
    const loadPendingFiles = async () => {
      ...
      const stubs = (
        await Promise.all(
          fileIds.map((id) => fileStorage.getPDFEliteFileStub(id as FileId)),
        )
      ...
      const materialized = await materializeServerStubs(stubs, {
        ...
        updateStub: actions.updatePDFEliteFileStub,
      });

      if (materialized.length > 0) {
        await actions.addPDFEliteFileStubs(materialized, { selectFiles: true });
        console.log(
