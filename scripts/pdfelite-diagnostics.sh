...
SCRIPT_NAME="PDFElite-diagnostics"
DEFAULT_DAYS=1
...
log "=== pdf-elite Diagnostics ==="
log "This tool collects a diagnostic bundle for pdf-elite."
...
if [ "$MODE" = "1" ]; then
  ...
  if prompt_yes_no "Include /tmp/pdf-elite?" "n"; then INCLUDE_TEMP=true; fi
...
if [ -f /etc/PDFElite_version ]; then
  write_kv "PDFElite_version_file" "$(tr -d '\r\n' < /etc/PDFElite_version)" "$SUMMARY"
fi
...
if [ "$INCLUDE_TEMP" = true ]; then
  log "Collecting /tmp/pdf-elite..."
  copy_dir_filtered "/tmp/pdf-elite" "$DATA_DIR" 0
fi
...
write_tree "/tmp/pdf-elite" "${TREE_DIR}/tmp-pdf-elite.txt"
