...
import type { PDFEliteFile } from "@app/types/fileContext";
...
export async function importShareLinkToWorkbench(
  ...
  if (isZipBundle(contentTypeValue, filename)) {
    ...
    if (bundle) {
      ...
      for (let i = 0; i < PDFEliteFiles.length; i += 1) {
        idMap.set(sortedEntries[i].logicalId, PDFEliteFiles[i].fileId);
      }
  ...
  const ids = PDFEliteFiles.map(
    (PDFEliteFile: PDFEliteFile) => PDFEliteFile.fileId,
  );
  if (ids.length > 0) {
    const sharedUpdates = {
      remoteStorageId: shareMetadata?.fileId,
    ...
    for (const fileId of ids) {
      actions.updatePDFEliteFileStub(fileId, sharedUpdates);
