...
const createTextStampImage = (
  ...
  ctx.textAlign = (config as any).textAlign || "left";
  ...
  if ((config as any).textAlign === "center") {
    xPosition = naturalWidth / 2;
  } else if ((config as any).textAlign === "right") {
...
>(function SignatureAPIBridge({ isSignMode = false }, ref) {
  ...
  const configureStampDefaults = useCallback(async () => {
    ...
    try {
      ...
      if (signatureConfig.signatureDataUrl) {
        const pdfSize = placementPreviewSize
          ? cssToPdfSize(placementPreviewSize)
          : undefined;
        lastStampImageRef.current = signatureConfig.signatureDataUrl;
        applyStampDefaults(
          signatureConfig.signatureDataUrl,
          `Digital Signature - ${(signatureConfig as any).reason || "Document signing"}`,
          pdfSize,
        );
        return;
      }
