...
public class pdfeliteUploadTool implements McpTool {

    private final ObjectMapper mapper;
    private final FileStorage fileStorage;

    public pdfeliteUploadTool(ObjectMapper mapper, FileStorage fileStorage) {
        this.mapper = mapper;
    ...
    public ObjectNode call(JsonNode arguments, McpCallContext context) {
        if (!context.hasScope("mcp.tools.write")) {
            return McpResponses.error(
                    mapper, "Insufficient scope: pdfelite_upload requires 'mcp.tools.write'.");
        }
        ...
        try {
            ...
            return McpResponses.text(
                    ...
                    "Stored '"
                            ...
                            + " bytes) as fileId="
                            + fileId
                            + ". Pass this fileId to a pdfelite operation's 'fileId' argument.");
        } catch (IOException e) {
