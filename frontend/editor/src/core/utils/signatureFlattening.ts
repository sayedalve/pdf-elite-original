...
export interface SignatureFlatteningResult {
  inputFileIds: FileId[];
  outputPDFEliteFile: PDFEliteFile;
  outputStub: PDFEliteFileStub;
}
...
export async function flattenSignatures(
  ...
  try {
    ...
    if (pdfArrayBuffer) {
        ...
        if (allFileIds.length > 0) {
          const fileIndex =
            activeFileIndex !== undefined && activeFileIndex < allFileIds.length
              ? activeFileIndex
              : 0;
          const fileStub = selectors.getPDFEliteFileStub(allFileIds[fileIndex]);
          const fileObject = selectors.getFile(allFileIds[fileIndex]);
          if (fileStub && fileObject) {
            currentFile = createPDFEliteFile(
              fileObject,
      ...
      const record = selectors.getPDFEliteFileStub(currentFile.fileId);
      if (!record) {
      ...
      const outputPDFEliteFile = createPDFEliteFile(signedFile, outputStub.id);
