...
interface FileGridProps {
  ...
  /** "Add to workspace". */
  onOpenFile: (file: PDFEliteFileStub) => void;
  onMoveFiles: (
    fileIds: FileId[],
  ...
  onSaveToServer?: (file: PDFEliteFileStub) => void;
  /** Open the version-history modal for a file (only when it has >1 version). */
  onVersionHistory?: (file: PDFEliteFileStub) => void;
...
function ListView({
  ...
  const visibleFileIds = useMemo(
    () =>
      entries
        .filter(
          (e): e is FilesPageEntry & { file: PDFEliteFileStub } =>
            e.kind === "file" && !!e.file,
        )
        .map((e) => e.file.id),
    [entries],
