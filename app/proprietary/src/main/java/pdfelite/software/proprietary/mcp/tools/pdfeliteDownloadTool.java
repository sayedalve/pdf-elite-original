...
public class pdfeliteDownloadTool implements McpTool {

    private final ObjectMapper mapper;
    private final FileStorage fileStorage;
    private final ApplicationProperties applicationProperties;

    public pdfeliteDownloadTool(
            ObjectMapper mapper,
    ...
    public ObjectNode call(JsonNode arguments, McpCallContext context) {
        if (!context.hasScope("mcp.tools.read")) {
            return McpResponses.error(
                    mapper, "Insufficient scope: pdfelite_download requires 'mcp.tools.read'.");
        }
