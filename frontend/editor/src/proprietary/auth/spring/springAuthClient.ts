...
const OAUTH_REDIRECT_COOKIE = "PDFElite_redirect_path";
...
export const POST_LOGIN_REDIRECT_STORAGE_KEY = "PDFElite_post_login_path";
...
class SpringAuthClient {
  ...
  async getSession(): Promise<{
    ...
    try {
      ...
      let token = localStorage.getItem("PDFElite_jwt");
      ...
      if (await platform().isDesktopSaaSAuthMode()) {
        ...
        if (tokenExpiry.expiresIn <= this.DESKTOP_SAAS_REFRESH_EARLY_SECONDS) {
          ...
          if (!refreshed) {
            localStorage.removeItem("PDFElite_jwt");
            return { data: { session: null }, error: null };
          }

          const refreshedToken = localStorage.getItem("PDFElite_jwt");
          if (!refreshedToken) {
            localStorage.removeItem("PDFElite_jwt");
            return { data: { session: null }, error: null };
          }
  ...
  async signInWithPassword(credentials: {
    ...
    try {
      ...
      localStorage.setItem("PDFElite_jwt", token);
   ...
   *
   * @param params.provider - Full auth path from backend (e.g., '/oauth2/authorization/google', '/saml2/authenticate/PDFElite')
    ...
    try {
      if (typeof window !== "undefined") {
        window.sessionStorage.setItem(
          "PDFElite_sso_auto_login_logged_out",
          "1",
        );
      }
