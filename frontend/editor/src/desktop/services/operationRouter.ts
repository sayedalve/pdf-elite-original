...
export class OperationRouter {
  ...
  async getBaseUrl(operation?: string): Promise<string> {
          ...
          if (!supportedLocally) {
            ...
            throw new Error(
              i18n.t(
                ...
                "This tool requires an account. Sign in to PDFElite Cloud or connect to a self-hosted server to use it.",
              ),
            );
          }
        }
    ...
    if (mode === "saas" && this.isSaaSBackendEndpoint(operation)) {
      if (!PDFElite_SAAS_BACKEND_API_URL) {
      ...
      if (status === "offline") {
        ...
        throw new Error(
          i18n.t(
            ...
            'Your pdf-elite server is offline and "{{endpoint}}" is not available on the local backend.',
