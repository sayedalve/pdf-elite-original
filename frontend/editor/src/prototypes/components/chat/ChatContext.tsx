 ...
 * {@code ProgressEvent} discriminated union (engine/src/PDFElite/contracts/progress.py)
...
interface AiWorkflowResultFile {
  /** PDFElite file ID — download with /api/v1/general/files/{fileId}. */
  fileId: string;
  fileName: string;
  contentType: string;
}
...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  // Download a File from the PDFElite files endpoint.
      ...
      if (sourceStubs.length > 0) {
        ...
        const stubs = files.map((file, i) =>
          isVersionMapping
            ...
            : createNewPDFEliteFileStub(file),
        );
        const PDFEliteFiles = files.map((file, i) =>
          createPDFEliteFile(file, stubs[i].id),
        );
