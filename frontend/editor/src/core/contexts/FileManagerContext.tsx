...
interface FileManagerContextValue {
  ...
  searchTerm: string;
  selectedFiles: PDFEliteFileStub[];
  filteredFiles: PDFEliteFileStub[];
  fileInputRef: React.RefObject<HTMLInputElement | null>;
  selectedFilesSet: Set<FileId>;
  expandedFileIds: Set<FileId>;
  fileGroups: Map<FileId, PDFEliteFileStub[]>;
  loadedHistoryFiles: Map<FileId, PDFEliteFileStub[]>;
  isLoading: boolean;
  ...
  onHistoryFileRemove: (file: PDFEliteFileStub) => void;
  onFileDoubleClick: (file: PDFEliteFileStub) => void;
  onOpenFiles: () => void;
  ...
  onDownloadSingle: (file: PDFEliteFileStub) => void;
  onToggleExpansion: (fileId: FileId) => void;
  onAddToRecents: (file: PDFEliteFileStub) => void;
  onUnzipFile: (file: PDFEliteFileStub) => Promise<void>;
  onMakeCopy: (file: PDFEliteFileStub) => Promise<void>;
  onNewFilesSelect: (files: File[]) => void;
  onGoogleDriveSelect: (files: File[]) => void;
  refreshRecentFiles: () => Promise<void>;

  // External props
  recentFiles: PDFEliteFileStub[];
...
interface FileManagerProviderProps {
  ...
  recentFiles: PDFEliteFileStub[];
  onRecentFilesSelected: (files: PDFEliteFileStub[]) => void; // For selecting stored files
...
export const FileManagerProvider: React.FC<FileManagerProviderProps> = ({
  ...
  const fileGroups = useMemo(() => {
    ...
    // Convert PDFEliteFileStub to FileRecord-like objects for grouping utility
  ...
  const requestDeleteChoice = useCallback(
    (file: PDFEliteFileStub): Promise<RemoteDeleteChoice> => {
  ...
  const handleFileSelect = useCallback(
    (file: PDFEliteFileStub, currentIndex: number, shiftKey?: boolean) => {
  ...
  const getSafeFilesToDelete = useCallback(
    (fileIds: FileId[], allStoredStubs: PDFEliteFileStub[]): FileId[] => {
  ...
  const performFileDelete = useCallback(
    async (fileToRemove: PDFEliteFileStub) => {
      ...
      const allStoredStubs = await fileStorage.getAllPDFEliteFileStubs();
      ...
      try {
        for (const fileId of filesToDelete) {
          await fileStorage.deletePDFEliteFile(fileId as FileId);
        }
      } catch (error) {
  ...
  const handleHistoryFileRemove = useCallback(
    async (fileToRemove: PDFEliteFileStub) => {
      ...
      // Delete safe files from IndexedDB
      try {
        await fileStorage.deletePDFEliteFile(deletedFileId);
      } catch (error) {
        console.error("Failed to delete files from chain:", error);
      }
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    const serverFiles = selectedFileIds
      ...
      .filter((f): f is PDFEliteFileStub => !!f && !!f.remoteStorageId);
    ...
    if (localIds.length > 0) {
      try {
        ...
        fileStorage
          .deleteMultiplePDFEliteFiles(safeIds)
  ...
  const handleDownloadSingle = useCallback(async (file: PDFEliteFileStub) => {
    try {
      ...
      if (!isCurrentlyExpanded) {
        ...
        if (
          ...
          try {
            ...
            const historyFiles: PDFEliteFileStub[] = [];

            // Find the original file

            // Collect only files in this specific branch (ancestors of current file)
            const chainFiles: PDFEliteFileStub[] = [];
            const allFiles = Array.from(fileMap.values());

            // Build a map for fast parent lookups
            const fileIdMap = new Map<FileId, PDFEliteFileStub>();
            ...
            // PDFEliteFileStubs already have all the data we need - no conversion required!
  ...
  const handleUnzipFile = useCallback(
    async (file: PDFEliteFileStub) => {
      try {
        ...
        const PDFEliteFile = await fileStorage.getPDFEliteFile(file.id);
        if (!PDFEliteFile) {
          return;
        }
