...
async function setupApp(page: Page): Promise<void> {
  ...
  // call used the pre-bootstrap `PDFElite` password and always 401'd.
...
async function getIDBFolderCount(page: Page): Promise<number> {
  return page.evaluate(async () => {
    return new Promise<number>((resolve) => {
      const req = indexedDB.open("pdf-elite-watched-folders");
      req.onsuccess = () => {
        const db = req.result;
...
async function clearAllIDBFolders(page: Page): Promise<void> {
  await page.evaluate(async () => {
    const dbNames = [
      "pdf-elite-watched-folders",
      "pdf-elite-folder-files",
      "pdf-elite-folder-run-state",
      "pdf-elite-retry-schedule",
      "pdf-elite-folder-seen-files",
      "pdf-elite-folder-directory-handles",
    ];
