...
type PDFEliteAnnotationCustomData = Record<string, unknown> & {
...
type PDFEliteAnnotationMetadata = {
  creationDate?: Date | number | string;
  customData?: PDFEliteAnnotationCustomData;
  M?: Date | number | string;
  modifiedDate?: Date | number | string;
};

type PDFEliteAnnotationPatch = Partial<PdfAnnotationObject> & {
  customData?: Record<string, unknown>;
};

function getPDFEliteAnnotationMetadata(
  ann: PdfAnnotationObject,
): PDFEliteAnnotationMetadata {
  return ann as PDFEliteAnnotationMetadata;
}

/** Format annotation date for display (e.g. "Mar 11, 6:05 PM"). */
function formatCommentDate(obj: PdfAnnotationObject): string {
  const metadata = getPDFEliteAnnotationMetadata(obj);
  const raw =
...
function isCommentAnnotation(ann: PdfAnnotationObject): boolean {
  const customData = getPDFEliteAnnotationMetadata(ann).customData;
...
function getRemoveCommentPatch(
  ann: PdfAnnotationObject,
): PDFEliteAnnotationPatch {
  const customData = {
    ...(getPDFEliteAnnotationMetadata(ann).customData ?? {}),
  };
