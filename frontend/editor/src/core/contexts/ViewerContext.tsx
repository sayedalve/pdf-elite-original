...
export interface ViewerContextType {
  ...
  registerImmediateSearchUpdate: (callback: () => void) => () => void;
  ...
  triggerImmediateSearchUpdate: () => void;
...
export const ViewerProvider: React.FC<ViewerProviderProps> = ({ children }) => {
  ...
  const setActiveFileIndex = useCallback(
    (index: number) => {
      ...
      if (file && isPDFEliteFile(file)) setActiveFileId(file.fileId);
    },
    [selectors],
  );
  ...
  const {
    register: registerImmediateSearchUpdate,
    trigger: triggerImmediateSearchInternal,
  ...
  // Phase 29: bridges call this after publishing new (deduped) search results
  // so subscribers (e.g. the workbench ViewerShell) re-read getSearchState()
  // and refresh the match count / active index deterministically, instead of
  // only updating as a side effect of an unrelated scroll/zoom notification.
  const triggerImmediateSearchUpdate = useCallback(() => {
    triggerImmediateSearchInternal();
  }, [triggerImmediateSearchInternal]);
...
export function useViewerZoom() {
  ...
  const [zoom, setZoom] = useState({
    percentage: viewer.getZoomState().zoomPercent,
    scale: viewer.getZoomState().currentZoom,
  });

  useEffect(() => {
    return viewer.registerImmediateZoomUpdate((percent) => {
      setZoom({ percentage: percent, scale: percent / 100 });
    });
  }, [viewer]);

  return { zoom, zoomActions: viewer.zoomActions };
}

export function useViewerScroll() {
  const viewer = useViewer();
  const [page, setPage] = useState({
    current: viewer.getScrollState().currentPage,
    total: viewer.getScrollState().totalPages,
  });

  useEffect(() => {
    return viewer.registerImmediateScrollUpdate((currentPage, totalPages) => {
      setPage({ current: currentPage, total: totalPages });
    });
  }, [viewer]);

  return { page, scrollActions: viewer.scrollActions };
}
