...
interface IndexedDBContextValue {
  ...
  loadMetadata: (fileId: FileId) => Promise<PDFEliteFileStub | null>;
  deleteFile: (fileId: FileId) => Promise<void>;

  // Batch operations
  loadAllMetadata: () => Promise<PDFEliteFileStub[]>;
  loadLeafMetadata: () => Promise<PDFEliteFileStub[]>; // Only leaf files for recent files list
...
export function IndexedDBProvider({ children }: IndexedDBProviderProps) {
  ...
  const saveFile = useCallback(
    async (
      ...
      const storedFile = await fileStorage.getPDFEliteFileStub(fileId);

      // Cache the file object for immediate reuse
      fileCache.current.set(fileId, { file, lastAccessed: Date.now() });
      evictLRUEntries();

      // Return PDFEliteFileStub from the stored file (no conversion needed)
      if (!storedFile) {
        throw new Error(
  ...
  const loadFile = useCallback(
    async (fileId: FileId): Promise<File | null> => {
      ...
      // Load from IndexedDB
      const storedFile = await fileStorage.getPDFEliteFile(fileId);
      if (!storedFile) return null;

      // PDFEliteFile is already a File object, no reconstruction needed
  ...
  const loadMetadata = useCallback(
    async (fileId: FileId): Promise<PDFEliteFileStub | null> => {
      // Load stub directly from storage service
      return await fileStorage.getPDFEliteFileStub(fileId);
    },
    [],
  );
  ...
  const deleteFile = useCallback(
    async (fileId: FileId): Promise<void> => {
      ...
      await fileStorage.deletePDFEliteFile(fileId);
      bumpRevision();
    },
    [bumpRevision],
  );
  ...
  const loadLeafMetadata = useCallback(async (): Promise<
    PDFEliteFileStub[]
  > => {
    const metadata = await fileStorage.getLeafPDFEliteFileStubs(); // Only get leaf files

    // All files are already PDFEliteFileStub objects, no processing needed
    return metadata;
  }, []);

  const loadAllMetadata = useCallback(async (): Promise<PDFEliteFileStub[]> => {
    const metadata = await fileStorage.getAllPDFEliteFileStubs();
  ...
  const deleteMultiple = useCallback(
    async (fileIds: FileId[]): Promise<void> => {
      ...
      await fileStorage.deleteMultiplePDFEliteFiles(fileIds);
      bumpRevision();
    },
    [bumpRevision],
  );
