...
const ConvertToEpubSettings = ({
  ...
  const handleDetectChaptersChange = (value: boolean) => {
    onParameterChange?.("epubOptions", {
      ...
      targetDevice:
        parameters.epubOptions?.targetDevice ?? "TABLET_PHONE_IMAGES",
      outputFormat:
        (parameters.epubOptions?.outputFormat ??
        parameters.toExtension === "azw3")
          ? "AZW3"
          : "EPUB",
    });
  };
  ...
  if (
    parameters.toExtension === "azw3" &&
    epubOptions.outputFormat !== "AZW3"
  ) {
    handleOutputFormatChange("AZW3");
  } else if (
    parameters.toExtension === "epub" &&
    epubOptions.outputFormat !== "EPUB"
  ) {
  ...
  return (
    <Stack gap="sm" data-testid="epub-settings">
      ...
      <Select
        ...
        description={t(
          "convert.epubOptions.targetDeviceDesc",
        ...
        data={[
          {
            ...
            label: t(
              "convert.epubOptions.tabletPhone",
              "Tablet/Phone (with images)",
            ),
          },
          {
            value: "KINDLE_EINK_TEXT",
            label: t(
              "convert.epubOptions.kindleEink",
              "Kindle e-Ink (text optimized)",
            ),
          },
        ]}
        comboboxProps={{
      ...
      <Select
        ...
        description={t(
          "convert.epubOptions.outputFormatDesc",
