...
Bucketing rules:
  Backend (JaCoCo package -> area):
    PDFElite/software/SPDF/**          -> core   (the main backend module)
    PDFElite/software/common/**        -> core   (shared infra, attributed to core)
    org/apache/pdfbox/**               -> core   (vendored helpers in common/core)
    PDFElite/software/proprietary/**   -> proprietary   (unless saas-flavoured)
    PDFElite/software/saas/**          -> saas
...
def _classify_backend(package_name: str) -> Optional[str]:
    ...
    if "desktop" in p:
        return "desktop"
    if p.startswith("PDFElite.software.saas"):
        return "saas"
    ...
    if p.startswith("PDFElite.software.proprietary") and ".saas" in p:
        return "saas"
    if p.startswith("PDFElite.software.proprietary"):
        return "proprietary"
    if (
        p.startswith("PDFElite.software.SPDF")
        or p.startswith("PDFElite.software.common")
