...
const mocks = vi.hoisted(() => ({
  ...
  getPDFEliteFileStub: vi.fn(),
  persistVersionedOutputs: vi.fn(),
  updateFileMetadata: vi.fn(),
  createPDFEliteFilesAndStubs: vi.fn(),
  addFiles: vi.fn(),
...
vi.mock("@app/services/fileStorage", () => ({
  fileStorage: {
    getPDFEliteFile: mocks.getPDFEliteFile,
    getPDFEliteFileStub: mocks.getPDFEliteFileStub,
...
vi.mock("@app/services/fileStubHelpers", () => ({
  createPDFEliteFilesAndStubs: mocks.createPDFEliteFilesAndStubs,
}));
...
beforeEach(() => {
  ...
  mocks.getPDFEliteFile.mockResolvedValue(
    new File(["x"], "doc.pdf", { type: "application/pdf" }),
  );
  mocks.getPDFEliteFileStub.mockResolvedValue(null);
...
describe("policy auto-run — 61-file batch through a Classification → Security chain", () => {
  ...
  it("does NOT re-open files that were closed while their runs were in flight", async () => {
    ...
    // The inputs still persist in storage, so getPDFEliteFileStub resolves them.
    mocks.getPDFEliteFileStub.mockResolvedValue({
      id: "storage",
      versionNumber: 1,
    });
    act(() => {
